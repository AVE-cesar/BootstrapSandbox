...
app.controller("ScdJobLogParController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "ScdJobLogParRestService", "ScdJobLogParRestSearchService", 
		"ScdJobLogParRestIndexService", "ScdJobLogParRestMassDeleteService",
			"$alert", "$timeout", function(scope, window, c, d, log, 
		scdJobLogParRestService, scdJobLogParRestSearchService, scdJobLogParRestIndexService, 
		scdJobLogParRestMassDeleteService, 
			alertService, timeoutService) {

/*log.info("inside ScdJobLogParController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside ScdJobLogParController");
	scdJobLogParRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside ScdJobLogParController");
	scdJobLogParRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside ScdJobLogParController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside ScdJobLogParController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside ScdJobLogParController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside ScdJobLogParController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside ScdJobLogParController");
...
scope.startElasticSearch = function(item) {
	...
	scdJobLogParRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	scdJobLogParRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-scdJobLogPar.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/scdJobLogPar/scdJobLogParSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	scdJobLogParRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			scdJobLogParRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your scdJobLogPar have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			scdJobLogParRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	scdJobLogParRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
						...
						/** main REST client for managing (4 CRUD calls) ScdJobLogPar entity */
app.factory('ScdJobLogParRestService', function ($resource) {
	return $resource('api/scdJobLogPars/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/scdJobLogPars/:scdJobLogIde,:parNme',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/scdJobLogPars/:scdJobLogIde,:parNme'},
		'update': { method:'PUT', url: 'api/scdJobLogPars/:scdJobLogIde,:parNme'},
		'delete': { method:'DELETE', url: 'api/scdJobLogPars/:scdJobLogIde,:parNme' }
	});
});

/** REST client for managing Elastic search calls on ScdJobLogPar entity */
app.factory('ScdJobLogParRestSearchService', function ($resource) {
	return $resource('api/scdJobLogPars/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing ScdJobLogPar entity */
app.factory('ScdJobLogParRestIndexService', function ($resource) {
	return $resource('api/scdJobLogPars/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on ScdJobLogPar entity */
app.factory('ScdJobLogParRestMassDeleteService', function ($resource) {
	return $resource('api/scdJobLogPars/mass/:id', {}, {
...
app.factory('ScdJobLogParRestInvRelationService', function ($resource) {
