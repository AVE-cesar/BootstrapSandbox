...
app.controller("AlertLogDetailController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "AlertLogDetailRestService", "AlertLogDetailRestSearchService", 
		"AlertLogDetailRestIndexService", "AlertLogDetailRestMassDeleteService",
							"AlertRestService",
													"$alert", "$timeout", function(scope, window, c, d, log, 
		alertLogDetailRestService, alertLogDetailRestSearchService, alertLogDetailRestIndexService, 
		alertLogDetailRestMassDeleteService, 
							alertRestService,
													alertService, timeoutService) {

/*log.info("inside AlertLogDetailController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside AlertLogDetailController");
	alertLogDetailRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside AlertLogDetailController");
	alertLogDetailRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside AlertLogDetailController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside AlertLogDetailController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside AlertLogDetailController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside AlertLogDetailController for page: " + (scope.pagination.totalPages - 1));
	scope.refreshByPage(scope.pagination.totalPages - 1, scope.totalElementsPerPage, false);		
};

				// fill Alert combo with data from server side
alertRestService.query({query: '*'}, function success(result){
	log.info("receiving Alert from server side");
	scope.alerts = result;
	log.info("Alert post refresh: " + result.length);
});
											
/** Clear the result grid */
scope.clear = function () {
	log.info("call method clear inside AlertLogDetailController");
...
scope.startElasticSearch = function(item) {
	...
	alertLogDetailRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	alertLogDetailRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-alertLogDetail.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/alertLogDetail/alertLogDetailSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	alertLogDetailRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your alertLogDetail have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your alertLogDetail haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		alertLogDetailRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		alertLogDetailRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			alertLogDetailRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your alertLogDetail have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			alertLogDetailRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	alertLogDetailRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) AlertLogDetail entity */
app.factory('AlertLogDetailRestService', function ($resource) {
	return $resource('api/alertLogDetails/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/alertLogDetails/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/alertLogDetails/:id'},
		'update': { method:'PUT', url: 'api/alertLogDetails/:id'},
		'delete': { method:'DELETE', url: 'api/alertLogDetails/:id' }
	});
});

/** REST client for managing Elastic search calls on AlertLogDetail entity */
app.factory('AlertLogDetailRestSearchService', function ($resource) {
	return $resource('api/alertLogDetails/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing AlertLogDetail entity */
app.factory('AlertLogDetailRestIndexService', function ($resource) {
	return $resource('api/alertLogDetails/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on AlertLogDetail entity */
app.factory('AlertLogDetailRestMassDeleteService', function ($resource) {
	return $resource('api/alertLogDetails/mass/:id', {}, {
...
app.factory('AlertLogDetailRestInvRelationService', function ($resource) {
