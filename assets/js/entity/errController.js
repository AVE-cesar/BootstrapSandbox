...
app.controller("ErrController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "ErrRestService", "ErrRestSearchService", 
		"ErrRestIndexService", "ErrRestMassDeleteService",
					"$alert", "$timeout", function(scope, window, c, d, log, 
		errRestService, errRestSearchService, errRestIndexService, 
		errRestMassDeleteService, 
					alertService, timeoutService) {

/*log.info("inside ErrController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside ErrController");
	errRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside ErrController");
	errRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside ErrController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside ErrController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside ErrController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside ErrController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside ErrController");
...
scope.startElasticSearch = function(item) {
	...
	errRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	errRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-err.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/err/errSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	errRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your err have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your err haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		errRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		errRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			errRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your err have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			errRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Err entity */
app.factory('ErrRestService', function ($resource) {
	return $resource('api/errs/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/errs/:id'},
		'update': { method:'PUT', url: 'api/errs/:id'},
		'delete': { method:'DELETE', url: 'api/errs/:id' }
	});
});

/** REST client for managing Elastic search calls on Err entity */
app.factory('ErrRestSearchService', function ($resource) {
	return $resource('api/errs/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Err entity */
app.factory('ErrRestIndexService', function ($resource) {
	return $resource('api/errs/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Err entity */
app.factory('ErrRestMassDeleteService', function ($resource) {
	return $resource('api/errs/mass/:id', {}, {
...
app.factory('ErrRestInvRelationService', function ($resource) {
