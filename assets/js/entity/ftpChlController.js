...
app.controller("FtpChlController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "FtpChlRestService", "FtpChlRestSearchService", 
		"FtpChlRestIndexService", "FtpChlRestMassDeleteService",
						/* Type of relation [FtpChl.id ==> Chl.id] not implemented yet !!!! */
															"$alert", "$timeout", function(scope, window, c, d, log, 
		ftpChlRestService, ftpChlRestSearchService, ftpChlRestIndexService, 
		ftpChlRestMassDeleteService, 
						/* Type of relation [FtpChl.id ==> Chl.id] not implemented yet !!!! */
															alertService, timeoutService) {

/*log.info("inside FtpChlController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside FtpChlController");
	ftpChlRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside FtpChlController");
	ftpChlRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside FtpChlController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside FtpChlController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside FtpChlController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside FtpChlController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside FtpChlController");
...
scope.startElasticSearch = function(item) {
	...
	ftpChlRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	ftpChlRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-ftpChl.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/ftpChl/ftpChlSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	ftpChlRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your ftpChl have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your ftpChl haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		ftpChlRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		ftpChlRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			ftpChlRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your ftpChl have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			ftpChlRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) FtpChl entity */
app.factory('FtpChlRestService', function ($resource) {
	return $resource('api/ftpChls/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/ftpChls/:id'},
		'update': { method:'PUT', url: 'api/ftpChls/:id'},
		'delete': { method:'DELETE', url: 'api/ftpChls/:id' }
	});
});

/** REST client for managing Elastic search calls on FtpChl entity */
app.factory('FtpChlRestSearchService', function ($resource) {
	return $resource('api/ftpChls/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing FtpChl entity */
app.factory('FtpChlRestIndexService', function ($resource) {
	return $resource('api/ftpChls/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on FtpChl entity */
app.factory('FtpChlRestMassDeleteService', function ($resource) {
	return $resource('api/ftpChls/mass/:id', {}, {
		'massDelete': { method: 'DELETE'}
	});
});

	/** REST client for managing linked entities for FtpChl entity */
	app.factory('FtpChlLinkedEntitiesService', function ($resource) {
								return $resource('api/chls/chlsByFtpChlsId/:id', {}, {
	        'queryChlsByFtpChlsId': {
	        	method: 'GET',
	        	url: 'api/chls/chlsByFtpChlsId/:id',
	        	isArray: true
	        }
				    })});
...
app.factory('FtpChlRestInvRelationService', function ($resource) {
