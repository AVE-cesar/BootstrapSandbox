...
app.controller("FieLnkController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "FieLnkRestService", "FieLnkRestSearchService", 
		"FieLnkRestIndexService", "FieLnkRestMassDeleteService",
							"FieRestService",
									"FieRestService",
									"RptSpnLnkRestService",
															"$alert", "$timeout", function(scope, window, c, d, log, 
		fieLnkRestService, fieLnkRestSearchService, fieLnkRestIndexService, 
		fieLnkRestMassDeleteService, 
							fieRestService,
									fieRestService,
									rptspnlnkRestService,
															alertService, timeoutService) {

/*log.info("inside FieLnkController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside FieLnkController");
	fieLnkRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside FieLnkController");
	fieLnkRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside FieLnkController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside FieLnkController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside FieLnkController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside FieLnkController for page: " + (scope.pagination.totalPages - 1));
	scope.refreshByPage(scope.pagination.totalPages - 1, scope.totalElementsPerPage, false);		
};

				// fill Fie combo with data from server side
fieRestService.query({query: '*'}, function success(result){
	log.info("receiving Fie from server side");
	scope.fies = result;
	log.info("Fie post refresh: " + result.length);
});
						// fill Fie combo with data from server side
fieRestService.query({query: '*'}, function success(result){
	log.info("receiving Fie from server side");
	...
	log.info("Fie post refresh: " + result.length);
						...
						// fill RptSpnLnk combo with data from server side
rptspnlnkRestService.query({query: '*'}, function success(result){
	log.info("receiving RptSpnLnk from server side");
	scope.rptspnlnks = result;
	log.info("RptSpnLnk post refresh: " + result.length);
});
													
/** Clear the result grid */
scope.clear = function () {
	log.info("call method clear inside FieLnkController");
...
scope.startElasticSearch = function(item) {
	...
	fieLnkRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	fieLnkRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-fieLnk.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/fieLnk/fieLnkSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	fieLnkRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your fieLnk have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your fieLnk haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		fieLnkRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		fieLnkRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			fieLnkRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your fieLnk have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			fieLnkRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) FieLnk entity */
app.factory('FieLnkRestService', function ($resource) {
	return $resource('api/fieLnks/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/fieLnks/:id'},
		'update': { method:'PUT', url: 'api/fieLnks/:id'},
		'delete': { method:'DELETE', url: 'api/fieLnks/:id' }
	});
});

/** REST client for managing Elastic search calls on FieLnk entity */
app.factory('FieLnkRestSearchService', function ($resource) {
	return $resource('api/fieLnks/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing FieLnk entity */
app.factory('FieLnkRestIndexService', function ($resource) {
	return $resource('api/fieLnks/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on FieLnk entity */
app.factory('FieLnkRestMassDeleteService', function ($resource) {
	return $resource('api/fieLnks/mass/:id', {}, {
...
app.factory('FieLnkRestInvRelationService', function ($resource) {
