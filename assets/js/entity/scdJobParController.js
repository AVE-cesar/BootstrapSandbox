...
app.controller("ScdJobParController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "ScdJobParRestService", "ScdJobParRestSearchService", 
		"ScdJobParRestIndexService", "ScdJobParRestMassDeleteService",
			"$alert", "$timeout", function(scope, window, c, d, log, 
		scdJobParRestService, scdJobParRestSearchService, scdJobParRestIndexService, 
		scdJobParRestMassDeleteService, 
			alertService, timeoutService) {

/*log.info("inside ScdJobParController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside ScdJobParController");
	scdJobParRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside ScdJobParController");
	scdJobParRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside ScdJobParController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside ScdJobParController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside ScdJobParController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside ScdJobParController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside ScdJobParController");
...
scope.startElasticSearch = function(item) {
	...
	scdJobParRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	scdJobParRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-scdJobPar.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/scdJobPar/scdJobParSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	scdJobParRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your scdJobPar have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your scdJobPar haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		scdJobParRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		scdJobParRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			scdJobParRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your scdJobPar have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			scdJobParRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	scdJobParRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
						...
						/** main REST client for managing (4 CRUD calls) ScdJobPar entity */
app.factory('ScdJobParRestService', function ($resource) {
	return $resource('api/scdJobPars/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/scdJobPars/:scdJobIde,:parNme',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/scdJobPars/:scdJobIde,:parNme'},
		'update': { method:'PUT', url: 'api/scdJobPars/:scdJobIde,:parNme'},
		'delete': { method:'DELETE', url: 'api/scdJobPars/:scdJobIde,:parNme' }
	});
});

/** REST client for managing Elastic search calls on ScdJobPar entity */
app.factory('ScdJobParRestSearchService', function ($resource) {
	return $resource('api/scdJobPars/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing ScdJobPar entity */
app.factory('ScdJobParRestIndexService', function ($resource) {
	return $resource('api/scdJobPars/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on ScdJobPar entity */
app.factory('ScdJobParRestMassDeleteService', function ($resource) {
	return $resource('api/scdJobPars/mass/:id', {}, {
...
app.factory('ScdJobParRestInvRelationService', function ($resource) {
