...
app.controller("DiffusionListAudController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "DiffusionListAudRestService", "DiffusionListAudRestSearchService", 
		"DiffusionListAudRestIndexService", "DiffusionListAudRestMassDeleteService",
									"$alert", "$timeout", function(scope, window, c, d, log, 
		diffusionListAudRestService, diffusionListAudRestSearchService, diffusionListAudRestIndexService, 
		diffusionListAudRestMassDeleteService, 
									alertService, timeoutService) {

/*log.info("inside DiffusionListAudController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside DiffusionListAudController");
	diffusionListAudRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside DiffusionListAudController");
	diffusionListAudRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside DiffusionListAudController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside DiffusionListAudController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside DiffusionListAudController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside DiffusionListAudController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside DiffusionListAudController");
...
scope.startElasticSearch = function(item) {
	...
	diffusionListAudRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	diffusionListAudRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-diffusionListAud.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/diffusionListAud/diffusionListAudSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	diffusionListAudRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			diffusionListAudRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your diffusionListAud have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
...
scope.index = function() {
	diffusionListAudRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) DiffusionListAud entity */
app.factory('DiffusionListAudRestService', function ($resource) {
	return $resource('api/diffusionListAuds/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/diffusionListAuds/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/diffusionListAuds/:id'},
		'update': { method:'PUT', url: 'api/diffusionListAuds/:id'},
		'delete': { method:'DELETE', url: 'api/diffusionListAuds/:id' }
	});
});

/** REST client for managing Elastic search calls on DiffusionListAud entity */
app.factory('DiffusionListAudRestSearchService', function ($resource) {
	return $resource('api/diffusionListAuds/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing DiffusionListAud entity */
app.factory('DiffusionListAudRestIndexService', function ($resource) {
	return $resource('api/diffusionListAuds/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on DiffusionListAud entity */
app.factory('DiffusionListAudRestMassDeleteService', function ($resource) {
	return $resource('api/diffusionListAuds/mass/:id', {}, {
...
app.factory('DiffusionListAudRestInvRelationService', function ($resource) {
