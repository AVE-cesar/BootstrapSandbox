...
app.controller("LogParController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "LogParRestService", "LogParRestSearchService", 
		"LogParRestIndexService", "LogParRestMassDeleteService",
						"$alert", "$timeout", function(scope, window, c, d, log, 
		logParRestService, logParRestSearchService, logParRestIndexService, 
		logParRestMassDeleteService, 
						alertService, timeoutService) {

/*log.info("inside LogParController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside LogParController");
	logParRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside LogParController");
	logParRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside LogParController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside LogParController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside LogParController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside LogParController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside LogParController");
...
scope.startElasticSearch = function(item) {
	...
	logParRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	logParRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-logPar.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/logPar/logParSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	logParRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your logPar have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your logPar haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		logParRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		logParRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			logParRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your logPar have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			logParRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) LogPar entity */
app.factory('LogParRestService', function ($resource) {
	return $resource('api/logPars/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/logPars/:id'},
		'update': { method:'PUT', url: 'api/logPars/:id'},
		'delete': { method:'DELETE', url: 'api/logPars/:id' }
	});
});

/** REST client for managing Elastic search calls on LogPar entity */
app.factory('LogParRestSearchService', function ($resource) {
	return $resource('api/logPars/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing LogPar entity */
app.factory('LogParRestIndexService', function ($resource) {
	return $resource('api/logPars/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on LogPar entity */
app.factory('LogParRestMassDeleteService', function ($resource) {
	return $resource('api/logPars/mass/:id', {}, {
...
app.factory('LogParRestInvRelationService', function ($resource) {
