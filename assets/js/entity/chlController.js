...
app.controller("ChlController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "ChlRestService", "ChlRestSearchService", 
		"ChlRestIndexService", "ChlRestMassDeleteService",
							"SpnRestService",
							"$alert", "$timeout", function(scope, window, c, d, log, 
		chlRestService, chlRestSearchService, chlRestIndexService, 
		chlRestMassDeleteService, 
							spnRestService,
							alertService, timeoutService) {

/*log.info("inside ChlController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside ChlController");
	chlRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside ChlController");
	chlRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside ChlController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside ChlController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside ChlController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside ChlController for page: " + (scope.pagination.totalPages - 1));
	scope.refreshByPage(scope.pagination.totalPages - 1, scope.totalElementsPerPage, false);		
};

				// fill Spn combo with data from server side
spnRestService.query({query: '*'}, function success(result){
	log.info("receiving Spn from server side");
	scope.spns = result;
	log.info("Spn post refresh: " + result.length);
});
					
/** Clear the result grid */
scope.clear = function () {
	log.info("call method clear inside ChlController");
...
scope.startElasticSearch = function(item) {
	...
	chlRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	chlRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-chl.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/chl/chlSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	chlRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your chl have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your chl haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		chlRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		chlRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			chlRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your chl have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			chlRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Chl entity */
app.factory('ChlRestService', function ($resource) {
	return $resource('api/chls/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/chls/:id'},
		'update': { method:'PUT', url: 'api/chls/:id'},
		'delete': { method:'DELETE', url: 'api/chls/:id' }
	});
});

/** REST client for managing Elastic search calls on Chl entity */
app.factory('ChlRestSearchService', function ($resource) {
	return $resource('api/chls/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Chl entity */
app.factory('ChlRestIndexService', function ($resource) {
	return $resource('api/chls/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Chl entity */
app.factory('ChlRestMassDeleteService', function ($resource) {
	return $resource('api/chls/mass/:id', {}, {
...
app.factory('ChlRestInvRelationService', function ($resource) {
