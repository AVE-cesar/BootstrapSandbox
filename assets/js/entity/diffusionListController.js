...
app.controller("DiffusionListController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "DiffusionListRestService", "DiffusionListRestSearchService", 
		"DiffusionListRestIndexService", "DiffusionListRestMassDeleteService",
					"$alert", "$timeout", function(scope, window, c, d, log, 
		diffusionListRestService, diffusionListRestSearchService, diffusionListRestIndexService, 
		diffusionListRestMassDeleteService, 
					alertService, timeoutService) {

/*log.info("inside DiffusionListController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside DiffusionListController");
	diffusionListRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside DiffusionListController");
	diffusionListRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside DiffusionListController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside DiffusionListController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside DiffusionListController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside DiffusionListController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside DiffusionListController");
...
scope.startElasticSearch = function(item) {
	...
	diffusionListRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	diffusionListRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-diffusionList.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/diffusionList/diffusionListSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	diffusionListRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			diffusionListRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your diffusionList have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
...
scope.index = function() {
	diffusionListRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) DiffusionList entity */
app.factory('DiffusionListRestService', function ($resource) {
	return $resource('api/diffusionLists/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/diffusionLists/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/diffusionLists/:id'},
		'update': { method:'PUT', url: 'api/diffusionLists/:id'},
		'delete': { method:'DELETE', url: 'api/diffusionLists/:id' }
	});
});

/** REST client for managing Elastic search calls on DiffusionList entity */
app.factory('DiffusionListRestSearchService', function ($resource) {
	return $resource('api/diffusionLists/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing DiffusionList entity */
app.factory('DiffusionListRestIndexService', function ($resource) {
	return $resource('api/diffusionLists/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on DiffusionList entity */
app.factory('DiffusionListRestMassDeleteService', function ($resource) {
	return $resource('api/diffusionLists/mass/:id', {}, {
...
app.factory('DiffusionListRestInvRelationService', function ($resource) {
