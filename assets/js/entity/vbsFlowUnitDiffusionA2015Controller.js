...
app.controller("VbsFlowUnitDiffusionA2015Controller", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "VbsFlowUnitDiffusionA2015RestService", "VbsFlowUnitDiffusionA2015RestSearchService", 
		"VbsFlowUnitDiffusionA2015RestIndexService", "VbsFlowUnitDiffusionA2015RestMassDeleteService",
					"$alert", "$timeout", function(scope, window, c, d, log, 
		vbsFlowUnitDiffusionA2015RestService, vbsFlowUnitDiffusionA2015RestSearchService, vbsFlowUnitDiffusionA2015RestIndexService, 
		vbsFlowUnitDiffusionA2015RestMassDeleteService, 
					alertService, timeoutService) {

/*log.info("inside VbsFlowUnitDiffusionA2015Controller, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside VbsFlowUnitDiffusionA2015Controller");
	vbsFlowUnitDiffusionA2015RestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside VbsFlowUnitDiffusionA2015Controller");
	vbsFlowUnitDiffusionA2015RestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside VbsFlowUnitDiffusionA2015Controller for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside VbsFlowUnitDiffusionA2015Controller for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside VbsFlowUnitDiffusionA2015Controller for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside VbsFlowUnitDiffusionA2015Controller for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside VbsFlowUnitDiffusionA2015Controller");
...
scope.startElasticSearch = function(item) {
	...
	vbsFlowUnitDiffusionA2015RestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	vbsFlowUnitDiffusionA2015RestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-vbsFlowUnitDiffusionA2015.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/vbsFlowUnitDiffusionA2015/vbsFlowUnitDiffusionA2015Search.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	vbsFlowUnitDiffusionA2015RestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			vbsFlowUnitDiffusionA2015RestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your vbsFlowUnitDiffusionA2015 have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			vbsFlowUnitDiffusionA2015RestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	vbsFlowUnitDiffusionA2015RestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) VbsFlowUnitDiffusionA2015 entity */
app.factory('VbsFlowUnitDiffusionA2015RestService', function ($resource) {
	return $resource('api/vbsFlowUnitDiffusionA2015s/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/vbsFlowUnitDiffusionA2015s/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/vbsFlowUnitDiffusionA2015s/:id'},
		'update': { method:'PUT', url: 'api/vbsFlowUnitDiffusionA2015s/:id'},
		'delete': { method:'DELETE', url: 'api/vbsFlowUnitDiffusionA2015s/:id' }
	});
});

/** REST client for managing Elastic search calls on VbsFlowUnitDiffusionA2015 entity */
app.factory('VbsFlowUnitDiffusionA2015RestSearchService', function ($resource) {
	return $resource('api/vbsFlowUnitDiffusionA2015s/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing VbsFlowUnitDiffusionA2015 entity */
app.factory('VbsFlowUnitDiffusionA2015RestIndexService', function ($resource) {
	return $resource('api/vbsFlowUnitDiffusionA2015s/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on VbsFlowUnitDiffusionA2015 entity */
app.factory('VbsFlowUnitDiffusionA2015RestMassDeleteService', function ($resource) {
	return $resource('api/vbsFlowUnitDiffusionA2015s/mass/:id', {}, {
...
app.factory('VbsFlowUnitDiffusionA2015RestInvRelationService', function ($resource) {
