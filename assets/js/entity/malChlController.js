...
app.controller("MalChlController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "MalChlRestService", "MalChlRestSearchService", 
		"MalChlRestIndexService", "MalChlRestMassDeleteService",
						/* Type of relation [MalChl.id ==> Chl.id] not implemented yet !!!! */
									"$alert", "$timeout", function(scope, window, c, d, log, 
		malChlRestService, malChlRestSearchService, malChlRestIndexService, 
		malChlRestMassDeleteService, 
						/* Type of relation [MalChl.id ==> Chl.id] not implemented yet !!!! */
									alertService, timeoutService) {

/*log.info("inside MalChlController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside MalChlController");
	malChlRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside MalChlController");
	malChlRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside MalChlController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside MalChlController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside MalChlController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside MalChlController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside MalChlController");
...
scope.startElasticSearch = function(item) {
	...
	malChlRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	malChlRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-malChl.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/malChl/malChlSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	malChlRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your malChl have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your malChl haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		malChlRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		malChlRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			malChlRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your malChl have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			malChlRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) MalChl entity */
app.factory('MalChlRestService', function ($resource) {
	return $resource('api/malChls/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/malChls/:id'},
		'update': { method:'PUT', url: 'api/malChls/:id'},
		'delete': { method:'DELETE', url: 'api/malChls/:id' }
	});
});

/** REST client for managing Elastic search calls on MalChl entity */
app.factory('MalChlRestSearchService', function ($resource) {
	return $resource('api/malChls/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing MalChl entity */
app.factory('MalChlRestIndexService', function ($resource) {
	return $resource('api/malChls/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on MalChl entity */
app.factory('MalChlRestMassDeleteService', function ($resource) {
	return $resource('api/malChls/mass/:id', {}, {
		'massDelete': { method: 'DELETE'}
	});
});

	/** REST client for managing linked entities for MalChl entity */
	app.factory('MalChlLinkedEntitiesService', function ($resource) {
								return $resource('api/chls/chlsByMalChlsId/:id', {}, {
	        'queryChlsByMalChlsId': {
	        	method: 'GET',
	        	url: 'api/chls/chlsByMalChlsId/:id',
	        	isArray: true
	        }
				    })});
...
app.factory('MalChlRestInvRelationService', function ($resource) {
