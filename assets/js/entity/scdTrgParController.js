...
app.controller("ScdTrgParController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "ScdTrgParRestService", "ScdTrgParRestSearchService", 
		"ScdTrgParRestIndexService", "ScdTrgParRestMassDeleteService",
			"$alert", "$timeout", function(scope, window, c, d, log, 
		scdTrgParRestService, scdTrgParRestSearchService, scdTrgParRestIndexService, 
		scdTrgParRestMassDeleteService, 
			alertService, timeoutService) {

/*log.info("inside ScdTrgParController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside ScdTrgParController");
	scdTrgParRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside ScdTrgParController");
	scdTrgParRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside ScdTrgParController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside ScdTrgParController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside ScdTrgParController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside ScdTrgParController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside ScdTrgParController");
...
scope.startElasticSearch = function(item) {
	...
	scdTrgParRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	scdTrgParRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-scdTrgPar.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/scdTrgPar/scdTrgParSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	scdTrgParRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your scdTrgPar have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your scdTrgPar haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		scdTrgParRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		scdTrgParRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			scdTrgParRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your scdTrgPar have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			scdTrgParRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	scdTrgParRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
						...
						/** main REST client for managing (4 CRUD calls) ScdTrgPar entity */
app.factory('ScdTrgParRestService', function ($resource) {
	return $resource('api/scdTrgPars/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/scdTrgPars/:scdJobIde,:parNme',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/scdTrgPars/:scdJobIde,:parNme'},
		'update': { method:'PUT', url: 'api/scdTrgPars/:scdJobIde,:parNme'},
		'delete': { method:'DELETE', url: 'api/scdTrgPars/:scdJobIde,:parNme' }
	});
});

/** REST client for managing Elastic search calls on ScdTrgPar entity */
app.factory('ScdTrgParRestSearchService', function ($resource) {
	return $resource('api/scdTrgPars/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing ScdTrgPar entity */
app.factory('ScdTrgParRestIndexService', function ($resource) {
	return $resource('api/scdTrgPars/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on ScdTrgPar entity */
app.factory('ScdTrgParRestMassDeleteService', function ($resource) {
	return $resource('api/scdTrgPars/mass/:id', {}, {
...
app.factory('ScdTrgParRestInvRelationService', function ($resource) {
