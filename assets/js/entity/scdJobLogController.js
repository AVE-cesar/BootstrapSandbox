...
app.controller("ScdJobLogController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "ScdJobLogRestService", "ScdJobLogRestSearchService", 
		"ScdJobLogRestIndexService", "ScdJobLogRestMassDeleteService",
							"ScdJobRestService",
										"$alert", "$timeout", function(scope, window, c, d, log, 
		scdJobLogRestService, scdJobLogRestSearchService, scdJobLogRestIndexService, 
		scdJobLogRestMassDeleteService, 
							scdjobRestService,
										alertService, timeoutService) {

/*log.info("inside ScdJobLogController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside ScdJobLogController");
	scdJobLogRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside ScdJobLogController");
	scdJobLogRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside ScdJobLogController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside ScdJobLogController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside ScdJobLogController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside ScdJobLogController for page: " + (scope.pagination.totalPages - 1));
	scope.refreshByPage(scope.pagination.totalPages - 1, scope.totalElementsPerPage, false);		
};

				// fill ScdJob combo with data from server side
scdjobRestService.query({query: '*'}, function success(result){
	log.info("receiving ScdJob from server side");
	scope.scdjobs = result;
	log.info("ScdJob post refresh: " + result.length);
});
								
/** Clear the result grid */
scope.clear = function () {
	log.info("call method clear inside ScdJobLogController");
...
scope.startElasticSearch = function(item) {
	...
	scdJobLogRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	scdJobLogRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-scdJobLog.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/scdJobLog/scdJobLogSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	scdJobLogRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your scdJobLog have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your scdJobLog haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		scdJobLogRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		scdJobLogRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			scdJobLogRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your scdJobLog have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			scdJobLogRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	scdJobLogRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) ScdJobLog entity */
app.factory('ScdJobLogRestService', function ($resource) {
	return $resource('api/scdJobLogs/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/scdJobLogs/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/scdJobLogs/:id'},
		'update': { method:'PUT', url: 'api/scdJobLogs/:id'},
		'delete': { method:'DELETE', url: 'api/scdJobLogs/:id' }
	});
});

/** REST client for managing Elastic search calls on ScdJobLog entity */
app.factory('ScdJobLogRestSearchService', function ($resource) {
	return $resource('api/scdJobLogs/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing ScdJobLog entity */
app.factory('ScdJobLogRestIndexService', function ($resource) {
	return $resource('api/scdJobLogs/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on ScdJobLog entity */
app.factory('ScdJobLogRestMassDeleteService', function ($resource) {
	return $resource('api/scdJobLogs/mass/:id', {}, {
...
app.factory('ScdJobLogRestInvRelationService', function ($resource) {
