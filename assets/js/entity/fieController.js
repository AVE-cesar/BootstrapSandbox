...
app.controller("FieController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "FieRestService", "FieRestSearchService", 
		"FieRestIndexService", "FieRestMassDeleteService",
													"$alert", "$timeout", function(scope, window, c, d, log, 
		fieRestService, fieRestSearchService, fieRestIndexService, 
		fieRestMassDeleteService, 
													alertService, timeoutService) {

/*log.info("inside FieController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside FieController");
	fieRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside FieController");
	fieRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside FieController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside FieController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside FieController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside FieController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside FieController");
...
scope.startElasticSearch = function(item) {
	...
	fieRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	fieRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-fie.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/fie/fieSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	fieRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your fie have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your fie haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		fieRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		fieRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			fieRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your fie have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			fieRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Fie entity */
app.factory('FieRestService', function ($resource) {
	return $resource('api/fies/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/fies/:id'},
		'update': { method:'PUT', url: 'api/fies/:id'},
		'delete': { method:'DELETE', url: 'api/fies/:id' }
	});
});

/** REST client for managing Elastic search calls on Fie entity */
app.factory('FieRestSearchService', function ($resource) {
	return $resource('api/fies/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Fie entity */
app.factory('FieRestIndexService', function ($resource) {
	return $resource('api/fies/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Fie entity */
app.factory('FieRestMassDeleteService', function ($resource) {
	return $resource('api/fies/mass/:id', {}, {
...
app.factory('FieRestInvRelationService', function ($resource) {
