...
app.controller("VbsFlowUnitSubFndA2015Controller", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "VbsFlowUnitSubFndA2015RestService", "VbsFlowUnitSubFndA2015RestSearchService", 
		"VbsFlowUnitSubFndA2015RestIndexService", "VbsFlowUnitSubFndA2015RestMassDeleteService",
																			"$alert", "$timeout", function(scope, window, c, d, log, 
		vbsFlowUnitSubFndA2015RestService, vbsFlowUnitSubFndA2015RestSearchService, vbsFlowUnitSubFndA2015RestIndexService, 
		vbsFlowUnitSubFndA2015RestMassDeleteService, 
																			alertService, timeoutService) {

/*log.info("inside VbsFlowUnitSubFndA2015Controller, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside VbsFlowUnitSubFndA2015Controller");
	vbsFlowUnitSubFndA2015RestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside VbsFlowUnitSubFndA2015Controller");
	vbsFlowUnitSubFndA2015RestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside VbsFlowUnitSubFndA2015Controller for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside VbsFlowUnitSubFndA2015Controller for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside VbsFlowUnitSubFndA2015Controller for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside VbsFlowUnitSubFndA2015Controller for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside VbsFlowUnitSubFndA2015Controller");
...
scope.startElasticSearch = function(item) {
	...
	vbsFlowUnitSubFndA2015RestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	vbsFlowUnitSubFndA2015RestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-vbsFlowUnitSubFndA2015.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/vbsFlowUnitSubFndA2015/vbsFlowUnitSubFndA2015Search.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	vbsFlowUnitSubFndA2015RestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your vbsFlowUnitSubFndA2015 have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your vbsFlowUnitSubFndA2015 haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		vbsFlowUnitSubFndA2015RestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		vbsFlowUnitSubFndA2015RestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			vbsFlowUnitSubFndA2015RestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your vbsFlowUnitSubFndA2015 have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			vbsFlowUnitSubFndA2015RestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	vbsFlowUnitSubFndA2015RestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) VbsFlowUnitSubFndA2015 entity */
app.factory('VbsFlowUnitSubFndA2015RestService', function ($resource) {
	return $resource('api/vbsFlowUnitSubFndA2015s/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/vbsFlowUnitSubFndA2015s/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/vbsFlowUnitSubFndA2015s/:id'},
		'update': { method:'PUT', url: 'api/vbsFlowUnitSubFndA2015s/:id'},
		'delete': { method:'DELETE', url: 'api/vbsFlowUnitSubFndA2015s/:id' }
	});
});

/** REST client for managing Elastic search calls on VbsFlowUnitSubFndA2015 entity */
app.factory('VbsFlowUnitSubFndA2015RestSearchService', function ($resource) {
	return $resource('api/vbsFlowUnitSubFndA2015s/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing VbsFlowUnitSubFndA2015 entity */
app.factory('VbsFlowUnitSubFndA2015RestIndexService', function ($resource) {
	return $resource('api/vbsFlowUnitSubFndA2015s/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on VbsFlowUnitSubFndA2015 entity */
app.factory('VbsFlowUnitSubFndA2015RestMassDeleteService', function ($resource) {
	return $resource('api/vbsFlowUnitSubFndA2015s/mass/:id', {}, {
...
app.factory('VbsFlowUnitSubFndA2015RestInvRelationService', function ($resource) {
