...
app.controller("XdpParController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "XdpParRestService", "XdpParRestSearchService", 
		"XdpParRestIndexService", "XdpParRestMassDeleteService",
				"$alert", "$timeout", function(scope, window, c, d, log, 
		xdpParRestService, xdpParRestSearchService, xdpParRestIndexService, 
		xdpParRestMassDeleteService, 
				alertService, timeoutService) {

/*log.info("inside XdpParController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside XdpParController");
	xdpParRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside XdpParController");
	xdpParRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside XdpParController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside XdpParController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside XdpParController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside XdpParController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside XdpParController");
...
scope.startElasticSearch = function(item) {
	...
	xdpParRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	xdpParRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-xdpPar.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/xdpPar/xdpParSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	xdpParRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your xdpPar have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your xdpPar haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		xdpParRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		xdpParRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			xdpParRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your xdpPar have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			xdpParRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) XdpPar entity */
app.factory('XdpParRestService', function ($resource) {
	return $resource('api/xdpPars/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/xdpPars/:id'},
		'update': { method:'PUT', url: 'api/xdpPars/:id'},
		'delete': { method:'DELETE', url: 'api/xdpPars/:id' }
	});
});

/** REST client for managing Elastic search calls on XdpPar entity */
app.factory('XdpParRestSearchService', function ($resource) {
	return $resource('api/xdpPars/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing XdpPar entity */
app.factory('XdpParRestIndexService', function ($resource) {
	return $resource('api/xdpPars/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on XdpPar entity */
app.factory('XdpParRestMassDeleteService', function ($resource) {
	return $resource('api/xdpPars/mass/:id', {}, {
...
app.factory('XdpParRestInvRelationService', function ($resource) {
