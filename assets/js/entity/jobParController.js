...
app.controller("JobParController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "JobParRestService", "JobParRestSearchService", 
		"JobParRestIndexService", "JobParRestMassDeleteService",
							"JobRestService",
							"$alert", "$timeout", function(scope, window, c, d, log, 
		jobParRestService, jobParRestSearchService, jobParRestIndexService, 
		jobParRestMassDeleteService, 
							jobRestService,
							alertService, timeoutService) {

/*log.info("inside JobParController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside JobParController");
	jobParRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside JobParController");
	jobParRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside JobParController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside JobParController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside JobParController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside JobParController for page: " + (scope.pagination.totalPages - 1));
	scope.refreshByPage(scope.pagination.totalPages - 1, scope.totalElementsPerPage, false);		
};

				// fill Job combo with data from server side
jobRestService.query({query: '*'}, function success(result){
	log.info("receiving Job from server side");
	scope.jobs = result;
	log.info("Job post refresh: " + result.length);
});
					
/** Clear the result grid */
scope.clear = function () {
	log.info("call method clear inside JobParController");
...
scope.startElasticSearch = function(item) {
	...
	jobParRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	jobParRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-jobPar.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/jobPar/jobParSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	jobParRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your jobPar have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your jobPar haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		jobParRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		jobParRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			jobParRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your jobPar have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			jobParRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) JobPar entity */
app.factory('JobParRestService', function ($resource) {
	return $resource('api/jobPars/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/jobPars/:id'},
		'update': { method:'PUT', url: 'api/jobPars/:id'},
		'delete': { method:'DELETE', url: 'api/jobPars/:id' }
	});
});

/** REST client for managing Elastic search calls on JobPar entity */
app.factory('JobParRestSearchService', function ($resource) {
	return $resource('api/jobPars/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing JobPar entity */
app.factory('JobParRestIndexService', function ($resource) {
	return $resource('api/jobPars/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on JobPar entity */
app.factory('JobParRestMassDeleteService', function ($resource) {
	return $resource('api/jobPars/mass/:id', {}, {
...
app.factory('JobParRestInvRelationService', function ($resource) {
