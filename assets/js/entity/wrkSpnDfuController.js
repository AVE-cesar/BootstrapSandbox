...
app.controller("WrkSpnDfuController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "WrkSpnDfuRestService", "WrkSpnDfuRestSearchService", 
		"WrkSpnDfuRestIndexService", "WrkSpnDfuRestMassDeleteService",
			"$alert", "$timeout", function(scope, window, c, d, log, 
		wrkSpnDfuRestService, wrkSpnDfuRestSearchService, wrkSpnDfuRestIndexService, 
		wrkSpnDfuRestMassDeleteService, 
			alertService, timeoutService) {

/*log.info("inside WrkSpnDfuController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside WrkSpnDfuController");
	wrkSpnDfuRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside WrkSpnDfuController");
	wrkSpnDfuRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside WrkSpnDfuController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside WrkSpnDfuController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside WrkSpnDfuController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside WrkSpnDfuController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside WrkSpnDfuController");
...
scope.startElasticSearch = function(item) {
	...
	wrkSpnDfuRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	wrkSpnDfuRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-wrkSpnDfu.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/wrkSpnDfu/wrkSpnDfuSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	wrkSpnDfuRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			wrkSpnDfuRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your wrkSpnDfu have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			wrkSpnDfuRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	wrkSpnDfuRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
						...
						/** main REST client for managing (4 CRUD calls) WrkSpnDfu entity */
app.factory('WrkSpnDfuRestService', function ($resource) {
	return $resource('api/wrkSpnDfus/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/wrkSpnDfus/:wrkDfuAppCde,:wrkDfuSpnIde',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/wrkSpnDfus/:wrkDfuAppCde,:wrkDfuSpnIde'},
		'update': { method:'PUT', url: 'api/wrkSpnDfus/:wrkDfuAppCde,:wrkDfuSpnIde'},
		'delete': { method:'DELETE', url: 'api/wrkSpnDfus/:wrkDfuAppCde,:wrkDfuSpnIde' }
	});
});

/** REST client for managing Elastic search calls on WrkSpnDfu entity */
app.factory('WrkSpnDfuRestSearchService', function ($resource) {
	return $resource('api/wrkSpnDfus/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing WrkSpnDfu entity */
app.factory('WrkSpnDfuRestIndexService', function ($resource) {
	return $resource('api/wrkSpnDfus/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on WrkSpnDfu entity */
app.factory('WrkSpnDfuRestMassDeleteService', function ($resource) {
	return $resource('api/wrkSpnDfus/mass/:id', {}, {
...
app.factory('WrkSpnDfuRestInvRelationService', function ($resource) {
