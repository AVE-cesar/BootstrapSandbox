...
app.controller("StdFieNbrController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "StdFieNbrRestService", "StdFieNbrRestSearchService", 
		"StdFieNbrRestIndexService", "StdFieNbrRestMassDeleteService",
			"$alert", "$timeout", function(scope, window, c, d, log, 
		stdFieNbrRestService, stdFieNbrRestSearchService, stdFieNbrRestIndexService, 
		stdFieNbrRestMassDeleteService, 
			alertService, timeoutService) {

/*log.info("inside StdFieNbrController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside StdFieNbrController");
	stdFieNbrRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside StdFieNbrController");
	stdFieNbrRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside StdFieNbrController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside StdFieNbrController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside StdFieNbrController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside StdFieNbrController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside StdFieNbrController");
...
scope.startElasticSearch = function(item) {
	...
	stdFieNbrRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	stdFieNbrRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-stdFieNbr.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/stdFieNbr/stdFieNbrSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	stdFieNbrRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			stdFieNbrRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your stdFieNbr have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			stdFieNbrRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	stdFieNbrRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
						...
						/** main REST client for managing (4 CRUD calls) StdFieNbr entity */
app.factory('StdFieNbrRestService', function ($resource) {
	return $resource('api/stdFieNbrs/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/stdFieNbrs/:rptSpnIde,:fndNbr,:ineDte',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/stdFieNbrs/:rptSpnIde,:fndNbr,:ineDte'},
		'update': { method:'PUT', url: 'api/stdFieNbrs/:rptSpnIde,:fndNbr,:ineDte'},
		'delete': { method:'DELETE', url: 'api/stdFieNbrs/:rptSpnIde,:fndNbr,:ineDte' }
	});
});

/** REST client for managing Elastic search calls on StdFieNbr entity */
app.factory('StdFieNbrRestSearchService', function ($resource) {
	return $resource('api/stdFieNbrs/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing StdFieNbr entity */
app.factory('StdFieNbrRestIndexService', function ($resource) {
	return $resource('api/stdFieNbrs/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on StdFieNbr entity */
app.factory('StdFieNbrRestMassDeleteService', function ($resource) {
	return $resource('api/stdFieNbrs/mass/:id', {}, {
...
app.factory('StdFieNbrRestInvRelationService', function ($resource) {
