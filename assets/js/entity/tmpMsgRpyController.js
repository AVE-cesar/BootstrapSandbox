...
app.controller("TmpMsgRpyController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "TmpMsgRpyRestService", "TmpMsgRpyRestSearchService", 
		"TmpMsgRpyRestIndexService", "TmpMsgRpyRestMassDeleteService",
							"$alert", "$timeout", function(scope, window, c, d, log, 
		tmpMsgRpyRestService, tmpMsgRpyRestSearchService, tmpMsgRpyRestIndexService, 
		tmpMsgRpyRestMassDeleteService, 
							alertService, timeoutService) {

/*log.info("inside TmpMsgRpyController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside TmpMsgRpyController");
	tmpMsgRpyRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside TmpMsgRpyController");
	tmpMsgRpyRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside TmpMsgRpyController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside TmpMsgRpyController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside TmpMsgRpyController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside TmpMsgRpyController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside TmpMsgRpyController");
...
scope.startElasticSearch = function(item) {
	...
	tmpMsgRpyRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	tmpMsgRpyRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-tmpMsgRpy.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/tmpMsgRpy/tmpMsgRpySearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	tmpMsgRpyRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your tmpMsgRpy have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your tmpMsgRpy haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		tmpMsgRpyRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		tmpMsgRpyRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			tmpMsgRpyRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your tmpMsgRpy have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			tmpMsgRpyRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	tmpMsgRpyRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) TmpMsgRpy entity */
app.factory('TmpMsgRpyRestService', function ($resource) {
	return $resource('api/tmpMsgRpies/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/tmpMsgRpies/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/tmpMsgRpies/:id'},
		'update': { method:'PUT', url: 'api/tmpMsgRpies/:id'},
		'delete': { method:'DELETE', url: 'api/tmpMsgRpies/:id' }
	});
});

/** REST client for managing Elastic search calls on TmpMsgRpy entity */
app.factory('TmpMsgRpyRestSearchService', function ($resource) {
	return $resource('api/tmpMsgRpies/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing TmpMsgRpy entity */
app.factory('TmpMsgRpyRestIndexService', function ($resource) {
	return $resource('api/tmpMsgRpies/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on TmpMsgRpy entity */
app.factory('TmpMsgRpyRestMassDeleteService', function ($resource) {
	return $resource('api/tmpMsgRpies/mass/:id', {}, {
...
app.factory('TmpMsgRpyRestInvRelationService', function ($resource) {
