...
app.controller("AttachmentsController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "AttachmentsRestService", "AttachmentsRestSearchService", 
		"AttachmentsRestIndexService", "AttachmentsRestMassDeleteService",
							"UploadsRestService",
										"$alert", "$timeout", function(scope, window, c, d, log, 
		attachmentsRestService, attachmentsRestSearchService, attachmentsRestIndexService, 
		attachmentsRestMassDeleteService, 
							uploadsRestService,
										alertService, timeoutService) {

/*log.info("inside AttachmentsController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside AttachmentsController");
	attachmentsRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside AttachmentsController");
	attachmentsRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside AttachmentsController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside AttachmentsController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside AttachmentsController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside AttachmentsController for page: " + (scope.pagination.totalPages - 1));
	scope.refreshByPage(scope.pagination.totalPages - 1, scope.totalElementsPerPage, false);		
};

				// fill Uploads combo with data from server side
uploadsRestService.query({query: '*'}, function success(result){
	log.info("receiving Uploads from server side");
	scope.uploadss = result;
	log.info("Uploads post refresh: " + result.length);
});
								
/** Clear the result grid */
scope.clear = function () {
	log.info("call method clear inside AttachmentsController");
...
scope.startElasticSearch = function(item) {
	...
	attachmentsRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	attachmentsRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-attachments.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/attachments/attachmentsSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	attachmentsRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your attachments have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your attachments haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		attachmentsRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		attachmentsRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			attachmentsRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your attachments have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			attachmentsRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	attachmentsRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Attachments entity */
app.factory('AttachmentsRestService', function ($resource) {
	return $resource('api/attachmentss/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/attachmentss/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/attachmentss/:id'},
		'update': { method:'PUT', url: 'api/attachmentss/:id'},
		'delete': { method:'DELETE', url: 'api/attachmentss/:id' }
	});
});

/** REST client for managing Elastic search calls on Attachments entity */
app.factory('AttachmentsRestSearchService', function ($resource) {
	return $resource('api/attachmentss/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Attachments entity */
app.factory('AttachmentsRestIndexService', function ($resource) {
	return $resource('api/attachmentss/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Attachments entity */
app.factory('AttachmentsRestMassDeleteService', function ($resource) {
	return $resource('api/attachmentss/mass/:id', {}, {
...
app.factory('AttachmentsRestInvRelationService', function ($resource) {
