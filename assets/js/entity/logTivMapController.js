...
app.controller("LogTivMapController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "LogTivMapRestService", "LogTivMapRestSearchService", 
		"LogTivMapRestIndexService", "LogTivMapRestMassDeleteService",
							"$alert", "$timeout", function(scope, window, c, d, log, 
		logTivMapRestService, logTivMapRestSearchService, logTivMapRestIndexService, 
		logTivMapRestMassDeleteService, 
							alertService, timeoutService) {

/*log.info("inside LogTivMapController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside LogTivMapController");
	logTivMapRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside LogTivMapController");
	logTivMapRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside LogTivMapController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside LogTivMapController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside LogTivMapController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside LogTivMapController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside LogTivMapController");
...
scope.startElasticSearch = function(item) {
	...
	logTivMapRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	logTivMapRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-logTivMap.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/logTivMap/logTivMapSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	logTivMapRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your logTivMap have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your logTivMap haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		logTivMapRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		logTivMapRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			logTivMapRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your logTivMap have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			logTivMapRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	logTivMapRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) LogTivMap entity */
app.factory('LogTivMapRestService', function ($resource) {
	return $resource('api/logTivMaps/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/logTivMaps/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/logTivMaps/:id'},
		'update': { method:'PUT', url: 'api/logTivMaps/:id'},
		'delete': { method:'DELETE', url: 'api/logTivMaps/:id' }
	});
});

/** REST client for managing Elastic search calls on LogTivMap entity */
app.factory('LogTivMapRestSearchService', function ($resource) {
	return $resource('api/logTivMaps/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing LogTivMap entity */
app.factory('LogTivMapRestIndexService', function ($resource) {
	return $resource('api/logTivMaps/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on LogTivMap entity */
app.factory('LogTivMapRestMassDeleteService', function ($resource) {
	return $resource('api/logTivMaps/mass/:id', {}, {
...
app.factory('LogTivMapRestInvRelationService', function ($resource) {
