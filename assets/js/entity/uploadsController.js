...
app.controller("UploadsController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "UploadsRestService", "UploadsRestSearchService", 
		"UploadsRestIndexService", "UploadsRestMassDeleteService",
																"$alert", "$timeout", function(scope, window, c, d, log, 
		uploadsRestService, uploadsRestSearchService, uploadsRestIndexService, 
		uploadsRestMassDeleteService, 
																alertService, timeoutService) {

/*log.info("inside UploadsController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside UploadsController");
	uploadsRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside UploadsController");
	uploadsRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside UploadsController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside UploadsController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside UploadsController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside UploadsController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside UploadsController");
...
scope.startElasticSearch = function(item) {
	...
	uploadsRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	uploadsRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-uploads.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/uploads/uploadsSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	uploadsRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your uploads have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your uploads haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		uploadsRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		uploadsRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			uploadsRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your uploads have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			uploadsRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Uploads entity */
app.factory('UploadsRestService', function ($resource) {
	return $resource('api/uploadss/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/uploadss/:id'},
		'update': { method:'PUT', url: 'api/uploadss/:id'},
		'delete': { method:'DELETE', url: 'api/uploadss/:id' }
	});
});

/** REST client for managing Elastic search calls on Uploads entity */
app.factory('UploadsRestSearchService', function ($resource) {
	return $resource('api/uploadss/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Uploads entity */
app.factory('UploadsRestIndexService', function ($resource) {
	return $resource('api/uploadss/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Uploads entity */
app.factory('UploadsRestMassDeleteService', function ($resource) {
	return $resource('api/uploadss/mass/:id', {}, {
...
app.factory('UploadsRestInvRelationService', function ($resource) {
