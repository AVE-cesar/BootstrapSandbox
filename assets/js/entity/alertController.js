...
app.controller("AlertController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "AlertRestService", "AlertRestSearchService", 
		"AlertRestIndexService", "AlertRestMassDeleteService",
												"$alert", "$timeout", function(scope, window, c, d, log, 
		alertRestService, alertRestSearchService, alertRestIndexService, 
		alertRestMassDeleteService, 
												alertService, timeoutService) {

/*log.info("inside AlertController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside AlertController");
	alertRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside AlertController");
	alertRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside AlertController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside AlertController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside AlertController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside AlertController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside AlertController");
...
scope.startElasticSearch = function(item) {
	...
	alertRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	alertRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-alert.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/alert/alertSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	alertRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your alert have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your alert haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		alertRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		alertRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			alertRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your alert have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			alertRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Alert entity */
app.factory('AlertRestService', function ($resource) {
	return $resource('api/alerts/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/alerts/:id'},
		'update': { method:'PUT', url: 'api/alerts/:id'},
		'delete': { method:'DELETE', url: 'api/alerts/:id' }
	});
});

/** REST client for managing Elastic search calls on Alert entity */
app.factory('AlertRestSearchService', function ($resource) {
	return $resource('api/alerts/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Alert entity */
app.factory('AlertRestIndexService', function ($resource) {
	return $resource('api/alerts/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Alert entity */
app.factory('AlertRestMassDeleteService', function ($resource) {
	return $resource('api/alerts/mass/:id', {}, {
...
app.factory('AlertRestInvRelationService', function ($resource) {
