...
app.controller("EdmExeLogController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "EdmExeLogRestService", "EdmExeLogRestSearchService", 
		"EdmExeLogRestIndexService", "EdmExeLogRestMassDeleteService",
													"$alert", "$timeout", function(scope, window, c, d, log, 
		edmExeLogRestService, edmExeLogRestSearchService, edmExeLogRestIndexService, 
		edmExeLogRestMassDeleteService, 
													alertService, timeoutService) {

/*log.info("inside EdmExeLogController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside EdmExeLogController");
	edmExeLogRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside EdmExeLogController");
	edmExeLogRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside EdmExeLogController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside EdmExeLogController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside EdmExeLogController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside EdmExeLogController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside EdmExeLogController");
...
scope.startElasticSearch = function(item) {
	...
	edmExeLogRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	edmExeLogRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-edmExeLog.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/edmExeLog/edmExeLogSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	edmExeLogRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your edmExeLog have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your edmExeLog haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		edmExeLogRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		edmExeLogRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			edmExeLogRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your edmExeLog have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			edmExeLogRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	edmExeLogRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) EdmExeLog entity */
app.factory('EdmExeLogRestService', function ($resource) {
	return $resource('api/edmExeLogs/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/edmExeLogs/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/edmExeLogs/:id'},
		'update': { method:'PUT', url: 'api/edmExeLogs/:id'},
		'delete': { method:'DELETE', url: 'api/edmExeLogs/:id' }
	});
});

/** REST client for managing Elastic search calls on EdmExeLog entity */
app.factory('EdmExeLogRestSearchService', function ($resource) {
	return $resource('api/edmExeLogs/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing EdmExeLog entity */
app.factory('EdmExeLogRestIndexService', function ($resource) {
	return $resource('api/edmExeLogs/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on EdmExeLog entity */
app.factory('EdmExeLogRestMassDeleteService', function ($resource) {
	return $resource('api/edmExeLogs/mass/:id', {}, {
...
app.factory('EdmExeLogRestInvRelationService', function ($resource) {
