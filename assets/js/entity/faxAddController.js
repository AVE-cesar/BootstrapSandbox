...
app.controller("FaxAddController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "FaxAddRestService", "FaxAddRestSearchService", 
		"FaxAddRestIndexService", "FaxAddRestMassDeleteService",
						"FaxChlRestService",
							"$alert", "$timeout", function(scope, window, c, d, log, 
		faxAddRestService, faxAddRestSearchService, faxAddRestIndexService, 
		faxAddRestMassDeleteService, 
						faxchlRestService,
							alertService, timeoutService) {

/*log.info("inside FaxAddController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside FaxAddController");
	faxAddRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside FaxAddController");
	faxAddRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside FaxAddController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside FaxAddController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside FaxAddController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside FaxAddController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside FaxAddController");
...
scope.startElasticSearch = function(item) {
	...
	faxAddRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	faxAddRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-faxAdd.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/faxAdd/faxAddSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	faxAddRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			faxAddRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your faxAdd have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			faxAddRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) FaxAdd entity */
app.factory('FaxAddRestService', function ($resource) {
	return $resource('api/faxAdds/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/faxAdds/:id'},
		'update': { method:'PUT', url: 'api/faxAdds/:id'},
		'delete': { method:'DELETE', url: 'api/faxAdds/:id' }
	});
});

/** REST client for managing Elastic search calls on FaxAdd entity */
app.factory('FaxAddRestSearchService', function ($resource) {
	return $resource('api/faxAdds/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing FaxAdd entity */
app.factory('FaxAddRestIndexService', function ($resource) {
	return $resource('api/faxAdds/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on FaxAdd entity */
app.factory('FaxAddRestMassDeleteService', function ($resource) {
	return $resource('api/faxAdds/mass/:id', {}, {
...
app.factory('FaxAddRestInvRelationService', function ($resource) {
