...
app.controller("JobParValController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "JobParValRestService", "JobParValRestSearchService", 
		"JobParValRestIndexService", "JobParValRestMassDeleteService",
					"$alert", "$timeout", function(scope, window, c, d, log, 
		jobParValRestService, jobParValRestSearchService, jobParValRestIndexService, 
		jobParValRestMassDeleteService, 
					alertService, timeoutService) {

/*log.info("inside JobParValController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside JobParValController");
	jobParValRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside JobParValController");
	jobParValRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside JobParValController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside JobParValController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside JobParValController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside JobParValController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside JobParValController");
...
scope.startElasticSearch = function(item) {
	...
	jobParValRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	jobParValRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-jobParVal.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/jobParVal/jobParValSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	jobParValRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your jobParVal have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your jobParVal haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		jobParValRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		jobParValRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			jobParValRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your jobParVal have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			jobParValRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	jobParValRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) JobParVal entity */
app.factory('JobParValRestService', function ($resource) {
	return $resource('api/jobParVals/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/jobParVals/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/jobParVals/:id'},
		'update': { method:'PUT', url: 'api/jobParVals/:id'},
		'delete': { method:'DELETE', url: 'api/jobParVals/:id' }
	});
});

/** REST client for managing Elastic search calls on JobParVal entity */
app.factory('JobParValRestSearchService', function ($resource) {
	return $resource('api/jobParVals/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing JobParVal entity */
app.factory('JobParValRestIndexService', function ($resource) {
	return $resource('api/jobParVals/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on JobParVal entity */
app.factory('JobParValRestMassDeleteService', function ($resource) {
	return $resource('api/jobParVals/mass/:id', {}, {
...
app.factory('JobParValRestInvRelationService', function ($resource) {
