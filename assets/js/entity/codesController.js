...
app.controller("CodesController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "CodesRestService", "CodesRestSearchService", 
		"CodesRestIndexService", "CodesRestMassDeleteService",
						"$alert", "$timeout", function(scope, window, c, d, log, 
		codesRestService, codesRestSearchService, codesRestIndexService, 
		codesRestMassDeleteService, 
						alertService, timeoutService) {

/*log.info("inside CodesController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside CodesController");
	codesRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside CodesController");
	codesRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside CodesController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside CodesController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside CodesController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside CodesController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside CodesController");
...
scope.startElasticSearch = function(item) {
	...
	codesRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	codesRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-codes.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/codes/codesSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	codesRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your codes have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your codes haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		codesRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		codesRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			codesRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your codes have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			codesRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Codes entity */
app.factory('CodesRestService', function ($resource) {
	return $resource('api/codess/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/codess/:id'},
		'update': { method:'PUT', url: 'api/codess/:id'},
		'delete': { method:'DELETE', url: 'api/codess/:id' }
	});
});

/** REST client for managing Elastic search calls on Codes entity */
app.factory('CodesRestSearchService', function ($resource) {
	return $resource('api/codess/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Codes entity */
app.factory('CodesRestIndexService', function ($resource) {
	return $resource('api/codess/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Codes entity */
app.factory('CodesRestMassDeleteService', function ($resource) {
	return $resource('api/codess/mass/:id', {}, {
...
app.factory('CodesRestInvRelationService', function ($resource) {
