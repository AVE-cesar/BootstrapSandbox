...
app.controller("JobController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "JobRestService", "JobRestSearchService", 
		"JobRestIndexService", "JobRestMassDeleteService",
						"$alert", "$timeout", function(scope, window, c, d, log, 
		jobRestService, jobRestSearchService, jobRestIndexService, 
		jobRestMassDeleteService, 
						alertService, timeoutService) {

/*log.info("inside JobController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside JobController");
	jobRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside JobController");
	jobRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside JobController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside JobController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside JobController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside JobController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside JobController");
...
scope.startElasticSearch = function(item) {
	...
	jobRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	jobRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-job.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/job/jobSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	jobRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your job have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your job haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		jobRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		jobRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			jobRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your job have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			jobRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Job entity */
app.factory('JobRestService', function ($resource) {
	return $resource('api/jobs/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/jobs/:id'},
		'update': { method:'PUT', url: 'api/jobs/:id'},
		'delete': { method:'DELETE', url: 'api/jobs/:id' }
	});
});

/** REST client for managing Elastic search calls on Job entity */
app.factory('JobRestSearchService', function ($resource) {
	return $resource('api/jobs/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Job entity */
app.factory('JobRestIndexService', function ($resource) {
	return $resource('api/jobs/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Job entity */
app.factory('JobRestMassDeleteService', function ($resource) {
	return $resource('api/jobs/mass/:id', {}, {
...
app.factory('JobRestInvRelationService', function ($resource) {
