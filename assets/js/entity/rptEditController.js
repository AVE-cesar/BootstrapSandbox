...
app.controller("RptEditController", ["$scope", "$window", "$aside", 
"$log", "RptRestService", 
		"RptRestInvRelationService", "$alert", "$timeout", "item", "mode", function(scope, window, aside, log, 
		rptRestService, 
								rptRestInvRelationService, alertService, timeoutService, item, mode) {
	
	log.info("inside RptEditController, mode: " + mode);
	log.info("inside RptEditController, item: " + item);
	scope.mode = mode;
	scope.item = item;
		...
		if (r == true) {
			if (item) {				
				// one item deletion mode
												rptRestService.delete({id: item.id}, function success(data) {
					scope.clear();
					alert('item deleted');
