...
app.controller("DiffusionListEditController", ["$scope", "$window", "$aside", 
"$log", "DiffusionListRestService", 
		"DiffusionListRestInvRelationService", "$alert", "$timeout", "item", "mode", function(scope, window, aside, log, 
		diffusionListRestService, 
					diffusionListRestInvRelationService, alertService, timeoutService, item, mode) {
	
	log.info("inside DiffusionListEditController, mode: " + mode);
	log.info("inside DiffusionListEditController, item: " + item);
	scope.mode = mode;
	scope.item = item;
		...
		if (r == true) {
			if (item) {				
				// one item deletion mode
												diffusionListRestService.delete({id: item.id}, function success(data) {
					scope.clear();
					alert('item deleted');
