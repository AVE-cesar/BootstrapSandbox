...
app.controller("InlGrpController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "InlGrpRestService", "InlGrpRestSearchService", 
		"InlGrpRestIndexService", "InlGrpRestMassDeleteService",
				"$alert", "$timeout", function(scope, window, c, d, log, 
		inlGrpRestService, inlGrpRestSearchService, inlGrpRestIndexService, 
		inlGrpRestMassDeleteService, 
				alertService, timeoutService) {

/*log.info("inside InlGrpController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside InlGrpController");
	inlGrpRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside InlGrpController");
	inlGrpRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside InlGrpController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside InlGrpController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside InlGrpController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside InlGrpController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside InlGrpController");
...
scope.startElasticSearch = function(item) {
	...
	inlGrpRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	inlGrpRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-inlGrp.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/inlGrp/inlGrpSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	inlGrpRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your inlGrp have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your inlGrp haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		inlGrpRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		inlGrpRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			inlGrpRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your inlGrp have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			inlGrpRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) InlGrp entity */
app.factory('InlGrpRestService', function ($resource) {
	return $resource('api/inlGrps/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/inlGrps/:id'},
		'update': { method:'PUT', url: 'api/inlGrps/:id'},
		'delete': { method:'DELETE', url: 'api/inlGrps/:id' }
	});
});

/** REST client for managing Elastic search calls on InlGrp entity */
app.factory('InlGrpRestSearchService', function ($resource) {
	return $resource('api/inlGrps/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing InlGrp entity */
app.factory('InlGrpRestIndexService', function ($resource) {
	return $resource('api/inlGrps/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on InlGrp entity */
app.factory('InlGrpRestMassDeleteService', function ($resource) {
	return $resource('api/inlGrps/mass/:id', {}, {
		'massDelete': { method: 'DELETE'}
	});
});


	/** REST client for managing linked entities for InlGrp entity */
	app.factory('InlGrpLinkedEntitiesService', function ($resource) {
								return $resource('api/usrIdes/usrIdesByInlGrpsId/:id', {}, {
	        'queryUsrIdesByInlGrpsId': {
	        	method: 'GET',
	        	url: 'api/usrIdes/usrIdesByInlGrpsId/:id',
	        	isArray: true
	        }
				    })});

/** REST client for managing inverse relation */
app.factory('InlGrpRestInvRelationService', function ($resource) {
