...
app.controller("DiffusionMessageController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "DiffusionMessageRestService", "DiffusionMessageRestSearchService", 
		"DiffusionMessageRestIndexService", "DiffusionMessageRestMassDeleteService",
												"$alert", "$timeout", function(scope, window, c, d, log, 
		diffusionMessageRestService, diffusionMessageRestSearchService, diffusionMessageRestIndexService, 
		diffusionMessageRestMassDeleteService, 
												alertService, timeoutService) {

/*log.info("inside DiffusionMessageController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside DiffusionMessageController");
	diffusionMessageRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside DiffusionMessageController");
	diffusionMessageRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside DiffusionMessageController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside DiffusionMessageController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside DiffusionMessageController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside DiffusionMessageController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside DiffusionMessageController");
...
scope.startElasticSearch = function(item) {
	...
	diffusionMessageRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	diffusionMessageRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-diffusionMessage.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/diffusionMessage/diffusionMessageSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	diffusionMessageRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your diffusionMessage have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your diffusionMessage haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		diffusionMessageRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		diffusionMessageRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			diffusionMessageRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your diffusionMessage have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			diffusionMessageRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	diffusionMessageRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) DiffusionMessage entity */
app.factory('DiffusionMessageRestService', function ($resource) {
	return $resource('api/diffusionMessages/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/diffusionMessages/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/diffusionMessages/:id'},
		'update': { method:'PUT', url: 'api/diffusionMessages/:id'},
		'delete': { method:'DELETE', url: 'api/diffusionMessages/:id' }
	});
});

/** REST client for managing Elastic search calls on DiffusionMessage entity */
app.factory('DiffusionMessageRestSearchService', function ($resource) {
	return $resource('api/diffusionMessages/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing DiffusionMessage entity */
app.factory('DiffusionMessageRestIndexService', function ($resource) {
	return $resource('api/diffusionMessages/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on DiffusionMessage entity */
app.factory('DiffusionMessageRestMassDeleteService', function ($resource) {
	return $resource('api/diffusionMessages/mass/:id', {}, {
...
app.factory('DiffusionMessageRestInvRelationService', function ($resource) {
