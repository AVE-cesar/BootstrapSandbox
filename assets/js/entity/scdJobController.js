...
app.controller("ScdJobController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "ScdJobRestService", "ScdJobRestSearchService", 
		"ScdJobRestIndexService", "ScdJobRestMassDeleteService",
										"$alert", "$timeout", function(scope, window, c, d, log, 
		scdJobRestService, scdJobRestSearchService, scdJobRestIndexService, 
		scdJobRestMassDeleteService, 
										alertService, timeoutService) {

/*log.info("inside ScdJobController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside ScdJobController");
	scdJobRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside ScdJobController");
	scdJobRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside ScdJobController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside ScdJobController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside ScdJobController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside ScdJobController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside ScdJobController");
...
scope.startElasticSearch = function(item) {
	...
	scdJobRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	scdJobRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-scdJob.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/scdJob/scdJobSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	scdJobRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your scdJob have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your scdJob haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		scdJobRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		scdJobRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			scdJobRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your scdJob have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			scdJobRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) ScdJob entity */
app.factory('ScdJobRestService', function ($resource) {
	return $resource('api/scdJobs/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/scdJobs/:id'},
		'update': { method:'PUT', url: 'api/scdJobs/:id'},
		'delete': { method:'DELETE', url: 'api/scdJobs/:id' }
	});
});

/** REST client for managing Elastic search calls on ScdJob entity */
app.factory('ScdJobRestSearchService', function ($resource) {
	return $resource('api/scdJobs/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing ScdJob entity */
app.factory('ScdJobRestIndexService', function ($resource) {
	return $resource('api/scdJobs/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on ScdJob entity */
app.factory('ScdJobRestMassDeleteService', function ($resource) {
	return $resource('api/scdJobs/mass/:id', {}, {
...
app.factory('ScdJobRestInvRelationService', function ($resource) {
