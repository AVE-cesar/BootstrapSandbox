...
app.controller("RptSpnLnkController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "RptSpnLnkRestService", "RptSpnLnkRestSearchService", 
		"RptSpnLnkRestIndexService", "RptSpnLnkRestMassDeleteService",
											"$alert", "$timeout", function(scope, window, c, d, log, 
		rptSpnLnkRestService, rptSpnLnkRestSearchService, rptSpnLnkRestIndexService, 
		rptSpnLnkRestMassDeleteService, 
											alertService, timeoutService) {

/*log.info("inside RptSpnLnkController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside RptSpnLnkController");
	rptSpnLnkRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside RptSpnLnkController");
	rptSpnLnkRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside RptSpnLnkController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside RptSpnLnkController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside RptSpnLnkController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside RptSpnLnkController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside RptSpnLnkController");
...
scope.startElasticSearch = function(item) {
	...
	rptSpnLnkRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	rptSpnLnkRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-rptSpnLnk.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/rptSpnLnk/rptSpnLnkSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	rptSpnLnkRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your rptSpnLnk have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your rptSpnLnk haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		rptSpnLnkRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		rptSpnLnkRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			rptSpnLnkRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your rptSpnLnk have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			rptSpnLnkRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	rptSpnLnkRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) RptSpnLnk entity */
app.factory('RptSpnLnkRestService', function ($resource) {
	return $resource('api/rptSpnLnks/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/rptSpnLnks/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/rptSpnLnks/:id'},
		'update': { method:'PUT', url: 'api/rptSpnLnks/:id'},
		'delete': { method:'DELETE', url: 'api/rptSpnLnks/:id' }
	});
});

/** REST client for managing Elastic search calls on RptSpnLnk entity */
app.factory('RptSpnLnkRestSearchService', function ($resource) {
	return $resource('api/rptSpnLnks/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing RptSpnLnk entity */
app.factory('RptSpnLnkRestIndexService', function ($resource) {
	return $resource('api/rptSpnLnks/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on RptSpnLnk entity */
app.factory('RptSpnLnkRestMassDeleteService', function ($resource) {
	return $resource('api/rptSpnLnks/mass/:id', {}, {
...
app.factory('RptSpnLnkRestInvRelationService', function ($resource) {
