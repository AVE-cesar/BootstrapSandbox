...
app.controller("VbsFlowAvant2015Controller", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "VbsFlowAvant2015RestService", "VbsFlowAvant2015RestSearchService", 
		"VbsFlowAvant2015RestIndexService", "VbsFlowAvant2015RestMassDeleteService",
													"$alert", "$timeout", function(scope, window, c, d, log, 
		vbsFlowAvant2015RestService, vbsFlowAvant2015RestSearchService, vbsFlowAvant2015RestIndexService, 
		vbsFlowAvant2015RestMassDeleteService, 
													alertService, timeoutService) {

/*log.info("inside VbsFlowAvant2015Controller, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside VbsFlowAvant2015Controller");
	vbsFlowAvant2015RestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside VbsFlowAvant2015Controller");
	vbsFlowAvant2015RestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside VbsFlowAvant2015Controller for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside VbsFlowAvant2015Controller for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside VbsFlowAvant2015Controller for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside VbsFlowAvant2015Controller for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside VbsFlowAvant2015Controller");
...
scope.startElasticSearch = function(item) {
	...
	vbsFlowAvant2015RestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	vbsFlowAvant2015RestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-vbsFlowAvant2015.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/vbsFlowAvant2015/vbsFlowAvant2015Search.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	vbsFlowAvant2015RestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your vbsFlowAvant2015 have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your vbsFlowAvant2015 haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		vbsFlowAvant2015RestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		vbsFlowAvant2015RestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			vbsFlowAvant2015RestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your vbsFlowAvant2015 have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			vbsFlowAvant2015RestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	vbsFlowAvant2015RestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) VbsFlowAvant2015 entity */
app.factory('VbsFlowAvant2015RestService', function ($resource) {
	return $resource('api/vbsFlowAvant2015s/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/vbsFlowAvant2015s/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/vbsFlowAvant2015s/:id'},
		'update': { method:'PUT', url: 'api/vbsFlowAvant2015s/:id'},
		'delete': { method:'DELETE', url: 'api/vbsFlowAvant2015s/:id' }
	});
});

/** REST client for managing Elastic search calls on VbsFlowAvant2015 entity */
app.factory('VbsFlowAvant2015RestSearchService', function ($resource) {
	return $resource('api/vbsFlowAvant2015s/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing VbsFlowAvant2015 entity */
app.factory('VbsFlowAvant2015RestIndexService', function ($resource) {
	return $resource('api/vbsFlowAvant2015s/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on VbsFlowAvant2015 entity */
app.factory('VbsFlowAvant2015RestMassDeleteService', function ($resource) {
	return $resource('api/vbsFlowAvant2015s/mass/:id', {}, {
...
app.factory('VbsFlowAvant2015RestInvRelationService', function ($resource) {
