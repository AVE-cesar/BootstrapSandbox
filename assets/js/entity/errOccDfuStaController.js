...
app.controller("ErrOccDfuStaController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "ErrOccDfuStaRestService", "ErrOccDfuStaRestSearchService", 
		"ErrOccDfuStaRestIndexService", "ErrOccDfuStaRestMassDeleteService",
							"ErrTrgRestService",
									"ErrOccRestService",
						"$alert", "$timeout", function(scope, window, c, d, log, 
		errOccDfuStaRestService, errOccDfuStaRestSearchService, errOccDfuStaRestIndexService, 
		errOccDfuStaRestMassDeleteService, 
							errtrgRestService,
									erroccRestService,
						alertService, timeoutService) {

/*log.info("inside ErrOccDfuStaController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside ErrOccDfuStaController");
	errOccDfuStaRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside ErrOccDfuStaController");
	errOccDfuStaRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside ErrOccDfuStaController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside ErrOccDfuStaController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside ErrOccDfuStaController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside ErrOccDfuStaController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside ErrOccDfuStaController");
...
scope.startElasticSearch = function(item) {
	...
	errOccDfuStaRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	errOccDfuStaRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-errOccDfuSta.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/errOccDfuSta/errOccDfuStaSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	errOccDfuStaRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			errOccDfuStaRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your errOccDfuSta have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
...
scope.index = function() {
	errOccDfuStaRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) ErrOccDfuSta entity */
app.factory('ErrOccDfuStaRestService', function ($resource) {
	return $resource('api/errOccDfuStas/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/errOccDfuStas/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/errOccDfuStas/:id'},
		'update': { method:'PUT', url: 'api/errOccDfuStas/:id'},
		'delete': { method:'DELETE', url: 'api/errOccDfuStas/:id' }
	});
});

/** REST client for managing Elastic search calls on ErrOccDfuSta entity */
app.factory('ErrOccDfuStaRestSearchService', function ($resource) {
	return $resource('api/errOccDfuStas/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing ErrOccDfuSta entity */
app.factory('ErrOccDfuStaRestIndexService', function ($resource) {
	return $resource('api/errOccDfuStas/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on ErrOccDfuSta entity */
app.factory('ErrOccDfuStaRestMassDeleteService', function ($resource) {
	return $resource('api/errOccDfuStas/mass/:id', {}, {
...
app.factory('ErrOccDfuStaRestInvRelationService', function ($resource) {
