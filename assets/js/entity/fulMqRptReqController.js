...
app.controller("FulMqRptReqController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "FulMqRptReqRestService", "FulMqRptReqRestSearchService", 
		"FulMqRptReqRestIndexService", "FulMqRptReqRestMassDeleteService",
																	"$alert", "$timeout", function(scope, window, c, d, log, 
		fulMqRptReqRestService, fulMqRptReqRestSearchService, fulMqRptReqRestIndexService, 
		fulMqRptReqRestMassDeleteService, 
																	alertService, timeoutService) {

/*log.info("inside FulMqRptReqController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside FulMqRptReqController");
	fulMqRptReqRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside FulMqRptReqController");
	fulMqRptReqRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside FulMqRptReqController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside FulMqRptReqController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside FulMqRptReqController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside FulMqRptReqController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside FulMqRptReqController");
...
scope.startElasticSearch = function(item) {
	...
	fulMqRptReqRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	fulMqRptReqRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-fulMqRptReq.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/fulMqRptReq/fulMqRptReqSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	fulMqRptReqRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your fulMqRptReq have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your fulMqRptReq haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		fulMqRptReqRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		fulMqRptReqRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			fulMqRptReqRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your fulMqRptReq have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			fulMqRptReqRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	fulMqRptReqRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) FulMqRptReq entity */
app.factory('FulMqRptReqRestService', function ($resource) {
	return $resource('api/fulMqRptReqs/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/fulMqRptReqs/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/fulMqRptReqs/:id'},
		'update': { method:'PUT', url: 'api/fulMqRptReqs/:id'},
		'delete': { method:'DELETE', url: 'api/fulMqRptReqs/:id' }
	});
});

/** REST client for managing Elastic search calls on FulMqRptReq entity */
app.factory('FulMqRptReqRestSearchService', function ($resource) {
	return $resource('api/fulMqRptReqs/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing FulMqRptReq entity */
app.factory('FulMqRptReqRestIndexService', function ($resource) {
	return $resource('api/fulMqRptReqs/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on FulMqRptReq entity */
app.factory('FulMqRptReqRestMassDeleteService', function ($resource) {
	return $resource('api/fulMqRptReqs/mass/:id', {}, {
...
app.factory('FulMqRptReqRestInvRelationService', function ($resource) {
