...
app.controller("DfuLogController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "DfuLogRestService", "DfuLogRestSearchService", 
		"DfuLogRestIndexService", "DfuLogRestMassDeleteService",
																		"$alert", "$timeout", function(scope, window, c, d, log, 
		dfuLogRestService, dfuLogRestSearchService, dfuLogRestIndexService, 
		dfuLogRestMassDeleteService, 
																		alertService, timeoutService) {

/*log.info("inside DfuLogController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside DfuLogController");
	dfuLogRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside DfuLogController");
	dfuLogRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside DfuLogController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside DfuLogController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside DfuLogController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside DfuLogController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside DfuLogController");
...
scope.startElasticSearch = function(item) {
	...
	dfuLogRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	dfuLogRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-dfuLog.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/dfuLog/dfuLogSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	dfuLogRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your dfuLog have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your dfuLog haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		dfuLogRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		dfuLogRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			dfuLogRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your dfuLog have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			dfuLogRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) DfuLog entity */
app.factory('DfuLogRestService', function ($resource) {
	return $resource('api/dfuLogs/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/dfuLogs/:id'},
		'update': { method:'PUT', url: 'api/dfuLogs/:id'},
		'delete': { method:'DELETE', url: 'api/dfuLogs/:id' }
	});
});

/** REST client for managing Elastic search calls on DfuLog entity */
app.factory('DfuLogRestSearchService', function ($resource) {
	return $resource('api/dfuLogs/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing DfuLog entity */
app.factory('DfuLogRestIndexService', function ($resource) {
	return $resource('api/dfuLogs/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on DfuLog entity */
app.factory('DfuLogRestMassDeleteService', function ($resource) {
	return $resource('api/dfuLogs/mass/:id', {}, {
...
app.factory('DfuLogRestInvRelationService', function ($resource) {
