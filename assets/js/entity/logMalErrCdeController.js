...
app.controller("LogMalErrCdeController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "LogMalErrCdeRestService", "LogMalErrCdeRestSearchService", 
		"LogMalErrCdeRestIndexService", "LogMalErrCdeRestMassDeleteService",
					"$alert", "$timeout", function(scope, window, c, d, log, 
		logMalErrCdeRestService, logMalErrCdeRestSearchService, logMalErrCdeRestIndexService, 
		logMalErrCdeRestMassDeleteService, 
					alertService, timeoutService) {

/*log.info("inside LogMalErrCdeController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside LogMalErrCdeController");
	logMalErrCdeRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside LogMalErrCdeController");
	logMalErrCdeRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside LogMalErrCdeController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside LogMalErrCdeController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside LogMalErrCdeController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside LogMalErrCdeController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside LogMalErrCdeController");
...
scope.startElasticSearch = function(item) {
	...
	logMalErrCdeRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	logMalErrCdeRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-logMalErrCde.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/logMalErrCde/logMalErrCdeSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	logMalErrCdeRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your logMalErrCde have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your logMalErrCde haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		logMalErrCdeRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		logMalErrCdeRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			logMalErrCdeRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your logMalErrCde have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			logMalErrCdeRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	logMalErrCdeRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) LogMalErrCde entity */
app.factory('LogMalErrCdeRestService', function ($resource) {
	return $resource('api/logMalErrCdes/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/logMalErrCdes/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/logMalErrCdes/:id'},
		'update': { method:'PUT', url: 'api/logMalErrCdes/:id'},
		'delete': { method:'DELETE', url: 'api/logMalErrCdes/:id' }
	});
});

/** REST client for managing Elastic search calls on LogMalErrCde entity */
app.factory('LogMalErrCdeRestSearchService', function ($resource) {
	return $resource('api/logMalErrCdes/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing LogMalErrCde entity */
app.factory('LogMalErrCdeRestIndexService', function ($resource) {
	return $resource('api/logMalErrCdes/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on LogMalErrCde entity */
app.factory('LogMalErrCdeRestMassDeleteService', function ($resource) {
	return $resource('api/logMalErrCdes/mass/:id', {}, {
...
app.factory('LogMalErrCdeRestInvRelationService', function ($resource) {
