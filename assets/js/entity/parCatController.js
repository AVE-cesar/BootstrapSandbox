...
app.controller("ParCatController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "ParCatRestService", "ParCatRestSearchService", 
		"ParCatRestIndexService", "ParCatRestMassDeleteService",
				"$alert", "$timeout", function(scope, window, c, d, log, 
		parCatRestService, parCatRestSearchService, parCatRestIndexService, 
		parCatRestMassDeleteService, 
				alertService, timeoutService) {

/*log.info("inside ParCatController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside ParCatController");
	parCatRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside ParCatController");
	parCatRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside ParCatController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside ParCatController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside ParCatController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside ParCatController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside ParCatController");
...
scope.startElasticSearch = function(item) {
	...
	parCatRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	parCatRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-parCat.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/parCat/parCatSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	parCatRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your parCat have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your parCat haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		parCatRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		parCatRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			parCatRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your parCat have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			parCatRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) ParCat entity */
app.factory('ParCatRestService', function ($resource) {
	return $resource('api/parCats/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/parCats/:id'},
		'update': { method:'PUT', url: 'api/parCats/:id'},
		'delete': { method:'DELETE', url: 'api/parCats/:id' }
	});
});

/** REST client for managing Elastic search calls on ParCat entity */
app.factory('ParCatRestSearchService', function ($resource) {
	return $resource('api/parCats/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing ParCat entity */
app.factory('ParCatRestIndexService', function ($resource) {
	return $resource('api/parCats/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on ParCat entity */
app.factory('ParCatRestMassDeleteService', function ($resource) {
	return $resource('api/parCats/mass/:id', {}, {
...
app.factory('ParCatRestInvRelationService', function ($resource) {
