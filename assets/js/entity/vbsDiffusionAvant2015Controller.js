...
app.controller("VbsDiffusionAvant2015Controller", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "VbsDiffusionAvant2015RestService", "VbsDiffusionAvant2015RestSearchService", 
		"VbsDiffusionAvant2015RestIndexService", "VbsDiffusionAvant2015RestMassDeleteService",
																					"$alert", "$timeout", function(scope, window, c, d, log, 
		vbsDiffusionAvant2015RestService, vbsDiffusionAvant2015RestSearchService, vbsDiffusionAvant2015RestIndexService, 
		vbsDiffusionAvant2015RestMassDeleteService, 
																					alertService, timeoutService) {

/*log.info("inside VbsDiffusionAvant2015Controller, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside VbsDiffusionAvant2015Controller");
	vbsDiffusionAvant2015RestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside VbsDiffusionAvant2015Controller");
	vbsDiffusionAvant2015RestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside VbsDiffusionAvant2015Controller for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside VbsDiffusionAvant2015Controller for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside VbsDiffusionAvant2015Controller for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside VbsDiffusionAvant2015Controller for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside VbsDiffusionAvant2015Controller");
...
scope.startElasticSearch = function(item) {
	...
	vbsDiffusionAvant2015RestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	vbsDiffusionAvant2015RestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-vbsDiffusionAvant2015.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/vbsDiffusionAvant2015/vbsDiffusionAvant2015Search.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	vbsDiffusionAvant2015RestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your vbsDiffusionAvant2015 have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your vbsDiffusionAvant2015 haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		vbsDiffusionAvant2015RestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		vbsDiffusionAvant2015RestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			vbsDiffusionAvant2015RestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your vbsDiffusionAvant2015 have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			vbsDiffusionAvant2015RestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	vbsDiffusionAvant2015RestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) VbsDiffusionAvant2015 entity */
app.factory('VbsDiffusionAvant2015RestService', function ($resource) {
	return $resource('api/vbsDiffusionAvant2015s/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/vbsDiffusionAvant2015s/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/vbsDiffusionAvant2015s/:id'},
		'update': { method:'PUT', url: 'api/vbsDiffusionAvant2015s/:id'},
		'delete': { method:'DELETE', url: 'api/vbsDiffusionAvant2015s/:id' }
	});
});

/** REST client for managing Elastic search calls on VbsDiffusionAvant2015 entity */
app.factory('VbsDiffusionAvant2015RestSearchService', function ($resource) {
	return $resource('api/vbsDiffusionAvant2015s/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing VbsDiffusionAvant2015 entity */
app.factory('VbsDiffusionAvant2015RestIndexService', function ($resource) {
	return $resource('api/vbsDiffusionAvant2015s/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on VbsDiffusionAvant2015 entity */
app.factory('VbsDiffusionAvant2015RestMassDeleteService', function ($resource) {
	return $resource('api/vbsDiffusionAvant2015s/mass/:id', {}, {
...
app.factory('VbsDiffusionAvant2015RestInvRelationService', function ($resource) {
