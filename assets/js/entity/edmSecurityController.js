...
app.controller("EdmSecurityController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "EdmSecurityRestService", "EdmSecurityRestSearchService", 
		"EdmSecurityRestIndexService", "EdmSecurityRestMassDeleteService",
												"$alert", "$timeout", function(scope, window, c, d, log, 
		edmSecurityRestService, edmSecurityRestSearchService, edmSecurityRestIndexService, 
		edmSecurityRestMassDeleteService, 
												alertService, timeoutService) {

/*log.info("inside EdmSecurityController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside EdmSecurityController");
	edmSecurityRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside EdmSecurityController");
	edmSecurityRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside EdmSecurityController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside EdmSecurityController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside EdmSecurityController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside EdmSecurityController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside EdmSecurityController");
...
scope.startElasticSearch = function(item) {
	...
	edmSecurityRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	edmSecurityRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-edmSecurity.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/edmSecurity/edmSecuritySearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	edmSecurityRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your edmSecurity have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your edmSecurity haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		edmSecurityRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		edmSecurityRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			edmSecurityRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your edmSecurity have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			edmSecurityRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	edmSecurityRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) EdmSecurity entity */
app.factory('EdmSecurityRestService', function ($resource) {
	return $resource('api/edmSecurities/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/edmSecurities/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/edmSecurities/:id'},
		'update': { method:'PUT', url: 'api/edmSecurities/:id'},
		'delete': { method:'DELETE', url: 'api/edmSecurities/:id' }
	});
});

/** REST client for managing Elastic search calls on EdmSecurity entity */
app.factory('EdmSecurityRestSearchService', function ($resource) {
	return $resource('api/edmSecurities/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing EdmSecurity entity */
app.factory('EdmSecurityRestIndexService', function ($resource) {
	return $resource('api/edmSecurities/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on EdmSecurity entity */
app.factory('EdmSecurityRestMassDeleteService', function ($resource) {
	return $resource('api/edmSecurities/mass/:id', {}, {
...
app.factory('EdmSecurityRestInvRelationService', function ($resource) {
