...
app.controller("AlertRptController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "AlertRptRestService", "AlertRptRestSearchService", 
		"AlertRptRestIndexService", "AlertRptRestMassDeleteService",
							"AlertRestService",
							"$alert", "$timeout", function(scope, window, c, d, log, 
		alertRptRestService, alertRptRestSearchService, alertRptRestIndexService, 
		alertRptRestMassDeleteService, 
							alertRestService,
							alertService, timeoutService) {

/*log.info("inside AlertRptController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside AlertRptController");
	alertRptRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside AlertRptController");
	alertRptRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside AlertRptController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside AlertRptController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside AlertRptController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside AlertRptController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside AlertRptController");
...
scope.startElasticSearch = function(item) {
	...
	alertRptRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	alertRptRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-alertRpt.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/alertRpt/alertRptSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	alertRptRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your alertRpt have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your alertRpt haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		alertRptRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		alertRptRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			alertRptRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your alertRpt have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			alertRptRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	alertRptRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) AlertRpt entity */
app.factory('AlertRptRestService', function ($resource) {
	return $resource('api/alertRpts/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/alertRpts/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/alertRpts/:id'},
		'update': { method:'PUT', url: 'api/alertRpts/:id'},
		'delete': { method:'DELETE', url: 'api/alertRpts/:id' }
	});
});

/** REST client for managing Elastic search calls on AlertRpt entity */
app.factory('AlertRptRestSearchService', function ($resource) {
	return $resource('api/alertRpts/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing AlertRpt entity */
app.factory('AlertRptRestIndexService', function ($resource) {
	return $resource('api/alertRpts/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on AlertRpt entity */
app.factory('AlertRptRestMassDeleteService', function ($resource) {
	return $resource('api/alertRpts/mass/:id', {}, {
...
app.factory('AlertRptRestInvRelationService', function ($resource) {
