...
app.controller("EdmChlController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "EdmChlRestService", "EdmChlRestSearchService", 
		"EdmChlRestIndexService", "EdmChlRestMassDeleteService",
										"$alert", "$timeout", function(scope, window, c, d, log, 
		edmChlRestService, edmChlRestSearchService, edmChlRestIndexService, 
		edmChlRestMassDeleteService, 
										alertService, timeoutService) {

/*log.info("inside EdmChlController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside EdmChlController");
	edmChlRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside EdmChlController");
	edmChlRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside EdmChlController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside EdmChlController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside EdmChlController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside EdmChlController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside EdmChlController");
...
scope.startElasticSearch = function(item) {
	...
	edmChlRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	edmChlRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-edmChl.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/edmChl/edmChlSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	edmChlRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your edmChl have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your edmChl haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		edmChlRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		edmChlRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			edmChlRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your edmChl have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			edmChlRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) EdmChl entity */
app.factory('EdmChlRestService', function ($resource) {
	return $resource('api/edmChls/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/edmChls/:id'},
		'update': { method:'PUT', url: 'api/edmChls/:id'},
		'delete': { method:'DELETE', url: 'api/edmChls/:id' }
	});
});

/** REST client for managing Elastic search calls on EdmChl entity */
app.factory('EdmChlRestSearchService', function ($resource) {
	return $resource('api/edmChls/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing EdmChl entity */
app.factory('EdmChlRestIndexService', function ($resource) {
	return $resource('api/edmChls/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on EdmChl entity */
app.factory('EdmChlRestMassDeleteService', function ($resource) {
	return $resource('api/edmChls/mass/:id', {}, {
...
app.factory('EdmChlRestInvRelationService', function ($resource) {
