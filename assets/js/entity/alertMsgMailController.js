...
app.controller("AlertMsgMailController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "AlertMsgMailRestService", "AlertMsgMailRestSearchService", 
		"AlertMsgMailRestIndexService", "AlertMsgMailRestMassDeleteService",
							"AlertMsgRestService",
							"$alert", "$timeout", function(scope, window, c, d, log, 
		alertMsgMailRestService, alertMsgMailRestSearchService, alertMsgMailRestIndexService, 
		alertMsgMailRestMassDeleteService, 
							alertmsgRestService,
							alertService, timeoutService) {

/*log.info("inside AlertMsgMailController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside AlertMsgMailController");
	alertMsgMailRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside AlertMsgMailController");
	alertMsgMailRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside AlertMsgMailController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside AlertMsgMailController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside AlertMsgMailController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside AlertMsgMailController for page: " + (scope.pagination.totalPages - 1));
	scope.refreshByPage(scope.pagination.totalPages - 1, scope.totalElementsPerPage, false);		
};

				// fill AlertMsg combo with data from server side
alertmsgRestService.query({query: '*'}, function success(result){
	log.info("receiving AlertMsg from server side");
	scope.alertmsgs = result;
	log.info("AlertMsg post refresh: " + result.length);
});
					
/** Clear the result grid */
scope.clear = function () {
	log.info("call method clear inside AlertMsgMailController");
...
scope.startElasticSearch = function(item) {
	...
	alertMsgMailRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	alertMsgMailRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-alertMsgMail.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/alertMsgMail/alertMsgMailSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	alertMsgMailRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your alertMsgMail have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your alertMsgMail haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		alertMsgMailRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		alertMsgMailRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			alertMsgMailRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your alertMsgMail have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			alertMsgMailRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	alertMsgMailRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) AlertMsgMail entity */
app.factory('AlertMsgMailRestService', function ($resource) {
	return $resource('api/alertMsgMails/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/alertMsgMails/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/alertMsgMails/:id'},
		'update': { method:'PUT', url: 'api/alertMsgMails/:id'},
		'delete': { method:'DELETE', url: 'api/alertMsgMails/:id' }
	});
});

/** REST client for managing Elastic search calls on AlertMsgMail entity */
app.factory('AlertMsgMailRestSearchService', function ($resource) {
	return $resource('api/alertMsgMails/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing AlertMsgMail entity */
app.factory('AlertMsgMailRestIndexService', function ($resource) {
	return $resource('api/alertMsgMails/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on AlertMsgMail entity */
app.factory('AlertMsgMailRestMassDeleteService', function ($resource) {
	return $resource('api/alertMsgMails/mass/:id', {}, {
...
app.factory('AlertMsgMailRestInvRelationService', function ($resource) {
