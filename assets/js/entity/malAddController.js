...
app.controller("MalAddController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "MalAddRestService", "MalAddRestSearchService", 
		"MalAddRestIndexService", "MalAddRestMassDeleteService",
						"MalChlRestService",
							"$alert", "$timeout", function(scope, window, c, d, log, 
		malAddRestService, malAddRestSearchService, malAddRestIndexService, 
		malAddRestMassDeleteService, 
						malchlRestService,
							alertService, timeoutService) {

/*log.info("inside MalAddController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside MalAddController");
	malAddRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside MalAddController");
	malAddRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside MalAddController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside MalAddController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside MalAddController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside MalAddController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside MalAddController");
...
scope.startElasticSearch = function(item) {
	...
	malAddRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	malAddRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-malAdd.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/malAdd/malAddSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	malAddRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			malAddRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your malAdd have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) MalAdd entity */
app.factory('MalAddRestService', function ($resource) {
	return $resource('api/malAdds/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/malAdds/:id'},
		'update': { method:'PUT', url: 'api/malAdds/:id'},
		'delete': { method:'DELETE', url: 'api/malAdds/:id' }
	});
});

/** REST client for managing Elastic search calls on MalAdd entity */
app.factory('MalAddRestSearchService', function ($resource) {
	return $resource('api/malAdds/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing MalAdd entity */
app.factory('MalAddRestIndexService', function ($resource) {
	return $resource('api/malAdds/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on MalAdd entity */
app.factory('MalAddRestMassDeleteService', function ($resource) {
	return $resource('api/malAdds/mass/:id', {}, {
...
app.factory('MalAddRestInvRelationService', function ($resource) {
