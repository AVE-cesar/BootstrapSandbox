...
app.controller("FaxChlController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "FaxChlRestService", "FaxChlRestSearchService", 
		"FaxChlRestIndexService", "FaxChlRestMassDeleteService",
						/* Type of relation [FaxChl.id ==> Chl.id] not implemented yet !!!! */
										"$alert", "$timeout", function(scope, window, c, d, log, 
		faxChlRestService, faxChlRestSearchService, faxChlRestIndexService, 
		faxChlRestMassDeleteService, 
						/* Type of relation [FaxChl.id ==> Chl.id] not implemented yet !!!! */
										alertService, timeoutService) {

/*log.info("inside FaxChlController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside FaxChlController");
	faxChlRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside FaxChlController");
	faxChlRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside FaxChlController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside FaxChlController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside FaxChlController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside FaxChlController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside FaxChlController");
...
scope.startElasticSearch = function(item) {
	...
	faxChlRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	faxChlRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-faxChl.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/faxChl/faxChlSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	faxChlRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your faxChl have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your faxChl haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		faxChlRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		faxChlRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			faxChlRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your faxChl have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			faxChlRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) FaxChl entity */
app.factory('FaxChlRestService', function ($resource) {
	return $resource('api/faxChls/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/faxChls/:id'},
		'update': { method:'PUT', url: 'api/faxChls/:id'},
		'delete': { method:'DELETE', url: 'api/faxChls/:id' }
	});
});

/** REST client for managing Elastic search calls on FaxChl entity */
app.factory('FaxChlRestSearchService', function ($resource) {
	return $resource('api/faxChls/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing FaxChl entity */
app.factory('FaxChlRestIndexService', function ($resource) {
	return $resource('api/faxChls/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on FaxChl entity */
app.factory('FaxChlRestMassDeleteService', function ($resource) {
	return $resource('api/faxChls/mass/:id', {}, {
		'massDelete': { method: 'DELETE'}
	});
});

	/** REST client for managing linked entities for FaxChl entity */
	app.factory('FaxChlLinkedEntitiesService', function ($resource) {
								return $resource('api/chls/chlsByFaxChlsId/:id', {}, {
	        'queryChlsByFaxChlsId': {
	        	method: 'GET',
	        	url: 'api/chls/chlsByFaxChlsId/:id',
	        	isArray: true
	        }
				    })});
...
app.factory('FaxChlRestInvRelationService', function ($resource) {
