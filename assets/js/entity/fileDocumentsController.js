...
app.controller("FileDocumentsController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "FileDocumentsRestService", "FileDocumentsRestSearchService", 
		"FileDocumentsRestIndexService", "FileDocumentsRestMassDeleteService",
				"$alert", "$timeout", function(scope, window, c, d, log, 
		fileDocumentsRestService, fileDocumentsRestSearchService, fileDocumentsRestIndexService, 
		fileDocumentsRestMassDeleteService, 
				alertService, timeoutService) {

/*log.info("inside FileDocumentsController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside FileDocumentsController");
	fileDocumentsRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside FileDocumentsController");
	fileDocumentsRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside FileDocumentsController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside FileDocumentsController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside FileDocumentsController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside FileDocumentsController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside FileDocumentsController");
...
scope.startElasticSearch = function(item) {
	...
	fileDocumentsRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	fileDocumentsRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-fileDocuments.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/fileDocuments/fileDocumentsSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	fileDocumentsRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your fileDocuments have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your fileDocuments haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		fileDocumentsRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		fileDocumentsRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			fileDocumentsRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your fileDocuments have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			fileDocumentsRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	fileDocumentsRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) FileDocuments entity */
app.factory('FileDocumentsRestService', function ($resource) {
	return $resource('api/fileDocumentss/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/fileDocumentss/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/fileDocumentss/:id'},
		'update': { method:'PUT', url: 'api/fileDocumentss/:id'},
		'delete': { method:'DELETE', url: 'api/fileDocumentss/:id' }
	});
});

/** REST client for managing Elastic search calls on FileDocuments entity */
app.factory('FileDocumentsRestSearchService', function ($resource) {
	return $resource('api/fileDocumentss/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing FileDocuments entity */
app.factory('FileDocumentsRestIndexService', function ($resource) {
	return $resource('api/fileDocumentss/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on FileDocuments entity */
app.factory('FileDocumentsRestMassDeleteService', function ($resource) {
	return $resource('api/fileDocumentss/mass/:id', {}, {
...
app.factory('FileDocumentsRestInvRelationService', function ($resource) {
