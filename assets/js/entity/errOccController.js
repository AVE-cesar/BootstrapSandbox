...
app.controller("ErrOccController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "ErrOccRestService", "ErrOccRestSearchService", 
		"ErrOccRestIndexService", "ErrOccRestMassDeleteService",
							"ErrRestService",
									"$alert", "$timeout", function(scope, window, c, d, log, 
		errOccRestService, errOccRestSearchService, errOccRestIndexService, 
		errOccRestMassDeleteService, 
							errRestService,
									alertService, timeoutService) {

/*log.info("inside ErrOccController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside ErrOccController");
	errOccRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside ErrOccController");
	errOccRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside ErrOccController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside ErrOccController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside ErrOccController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside ErrOccController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside ErrOccController");
...
scope.startElasticSearch = function(item) {
	...
	errOccRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	errOccRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-errOcc.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/errOcc/errOccSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	errOccRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your errOcc have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your errOcc haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		errOccRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		errOccRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			errOccRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your errOcc have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			errOccRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) ErrOcc entity */
app.factory('ErrOccRestService', function ($resource) {
	return $resource('api/errOccs/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/errOccs/:id'},
		'update': { method:'PUT', url: 'api/errOccs/:id'},
		'delete': { method:'DELETE', url: 'api/errOccs/:id' }
	});
});

/** REST client for managing Elastic search calls on ErrOcc entity */
app.factory('ErrOccRestSearchService', function ($resource) {
	return $resource('api/errOccs/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing ErrOcc entity */
app.factory('ErrOccRestIndexService', function ($resource) {
	return $resource('api/errOccs/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on ErrOcc entity */
app.factory('ErrOccRestMassDeleteService', function ($resource) {
	return $resource('api/errOccs/mass/:id', {}, {
...
app.factory('ErrOccRestInvRelationService', function ($resource) {
