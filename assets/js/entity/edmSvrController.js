...
app.controller("EdmSvrController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "EdmSvrRestService", "EdmSvrRestSearchService", 
		"EdmSvrRestIndexService", "EdmSvrRestMassDeleteService",
											"$alert", "$timeout", function(scope, window, c, d, log, 
		edmSvrRestService, edmSvrRestSearchService, edmSvrRestIndexService, 
		edmSvrRestMassDeleteService, 
											alertService, timeoutService) {

/*log.info("inside EdmSvrController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside EdmSvrController");
	edmSvrRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside EdmSvrController");
	edmSvrRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside EdmSvrController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside EdmSvrController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside EdmSvrController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside EdmSvrController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside EdmSvrController");
...
scope.startElasticSearch = function(item) {
	...
	edmSvrRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	edmSvrRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-edmSvr.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/edmSvr/edmSvrSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	edmSvrRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your edmSvr have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your edmSvr haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		edmSvrRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		edmSvrRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			edmSvrRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your edmSvr have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			edmSvrRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) EdmSvr entity */
app.factory('EdmSvrRestService', function ($resource) {
	return $resource('api/edmSvrs/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/edmSvrs/:id'},
		'update': { method:'PUT', url: 'api/edmSvrs/:id'},
		'delete': { method:'DELETE', url: 'api/edmSvrs/:id' }
	});
});

/** REST client for managing Elastic search calls on EdmSvr entity */
app.factory('EdmSvrRestSearchService', function ($resource) {
	return $resource('api/edmSvrs/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing EdmSvr entity */
app.factory('EdmSvrRestIndexService', function ($resource) {
	return $resource('api/edmSvrs/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on EdmSvr entity */
app.factory('EdmSvrRestMassDeleteService', function ($resource) {
	return $resource('api/edmSvrs/mass/:id', {}, {
...
app.factory('EdmSvrRestInvRelationService', function ($resource) {
