...
app.controller("InfoRouteController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "InfoRouteRestService", "InfoRouteRestSearchService", 
		"InfoRouteRestIndexService", "InfoRouteRestMassDeleteService",
																"$alert", "$timeout", function(scope, window, c, d, log, 
		infoRouteRestService, infoRouteRestSearchService, infoRouteRestIndexService, 
		infoRouteRestMassDeleteService, 
																alertService, timeoutService) {

/*log.info("inside InfoRouteController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside InfoRouteController");
	infoRouteRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside InfoRouteController");
	infoRouteRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside InfoRouteController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside InfoRouteController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside InfoRouteController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside InfoRouteController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside InfoRouteController");
...
scope.startElasticSearch = function(item) {
	...
	infoRouteRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	infoRouteRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-infoRoute.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/infoRoute/infoRouteSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	infoRouteRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your infoRoute have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your infoRoute haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		infoRouteRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		infoRouteRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			infoRouteRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your infoRoute have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			infoRouteRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	infoRouteRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) InfoRoute entity */
app.factory('InfoRouteRestService', function ($resource) {
	return $resource('api/infoRoutes/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/infoRoutes/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/infoRoutes/:id'},
		'update': { method:'PUT', url: 'api/infoRoutes/:id'},
		'delete': { method:'DELETE', url: 'api/infoRoutes/:id' }
	});
});

/** REST client for managing Elastic search calls on InfoRoute entity */
app.factory('InfoRouteRestSearchService', function ($resource) {
	return $resource('api/infoRoutes/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing InfoRoute entity */
app.factory('InfoRouteRestIndexService', function ($resource) {
	return $resource('api/infoRoutes/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on InfoRoute entity */
app.factory('InfoRouteRestMassDeleteService', function ($resource) {
	return $resource('api/infoRoutes/mass/:id', {}, {
...
app.factory('InfoRouteRestInvRelationService', function ($resource) {
