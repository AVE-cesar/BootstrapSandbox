...
app.controller("VbsFlowUnitTransAvant2015Controller", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "VbsFlowUnitTransAvant2015RestService", "VbsFlowUnitTransAvant2015RestSearchService", 
		"VbsFlowUnitTransAvant2015RestIndexService", "VbsFlowUnitTransAvant2015RestMassDeleteService",
					"$alert", "$timeout", function(scope, window, c, d, log, 
		vbsFlowUnitTransAvant2015RestService, vbsFlowUnitTransAvant2015RestSearchService, vbsFlowUnitTransAvant2015RestIndexService, 
		vbsFlowUnitTransAvant2015RestMassDeleteService, 
					alertService, timeoutService) {

/*log.info("inside VbsFlowUnitTransAvant2015Controller, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside VbsFlowUnitTransAvant2015Controller");
	vbsFlowUnitTransAvant2015RestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside VbsFlowUnitTransAvant2015Controller");
	vbsFlowUnitTransAvant2015RestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside VbsFlowUnitTransAvant2015Controller for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside VbsFlowUnitTransAvant2015Controller for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside VbsFlowUnitTransAvant2015Controller for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside VbsFlowUnitTransAvant2015Controller for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside VbsFlowUnitTransAvant2015Controller");
...
scope.startElasticSearch = function(item) {
	...
	vbsFlowUnitTransAvant2015RestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	vbsFlowUnitTransAvant2015RestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-vbsFlowUnitTransAvant2015.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/vbsFlowUnitTransAvant2015/vbsFlowUnitTransAvant2015Search.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	vbsFlowUnitTransAvant2015RestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your vbsFlowUnitTransAvant2015 have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your vbsFlowUnitTransAvant2015 haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		vbsFlowUnitTransAvant2015RestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		vbsFlowUnitTransAvant2015RestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			vbsFlowUnitTransAvant2015RestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your vbsFlowUnitTransAvant2015 have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			vbsFlowUnitTransAvant2015RestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	vbsFlowUnitTransAvant2015RestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) VbsFlowUnitTransAvant2015 entity */
app.factory('VbsFlowUnitTransAvant2015RestService', function ($resource) {
	return $resource('api/vbsFlowUnitTransAvant2015s/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/vbsFlowUnitTransAvant2015s/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/vbsFlowUnitTransAvant2015s/:id'},
		'update': { method:'PUT', url: 'api/vbsFlowUnitTransAvant2015s/:id'},
		'delete': { method:'DELETE', url: 'api/vbsFlowUnitTransAvant2015s/:id' }
	});
});

/** REST client for managing Elastic search calls on VbsFlowUnitTransAvant2015 entity */
app.factory('VbsFlowUnitTransAvant2015RestSearchService', function ($resource) {
	return $resource('api/vbsFlowUnitTransAvant2015s/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing VbsFlowUnitTransAvant2015 entity */
app.factory('VbsFlowUnitTransAvant2015RestIndexService', function ($resource) {
	return $resource('api/vbsFlowUnitTransAvant2015s/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on VbsFlowUnitTransAvant2015 entity */
app.factory('VbsFlowUnitTransAvant2015RestMassDeleteService', function ($resource) {
	return $resource('api/vbsFlowUnitTransAvant2015s/mass/:id', {}, {
...
app.factory('VbsFlowUnitTransAvant2015RestInvRelationService', function ($resource) {
