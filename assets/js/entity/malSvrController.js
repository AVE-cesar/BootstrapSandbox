...
app.controller("MalSvrController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "MalSvrRestService", "MalSvrRestSearchService", 
		"MalSvrRestIndexService", "MalSvrRestMassDeleteService",
								"$alert", "$timeout", function(scope, window, c, d, log, 
		malSvrRestService, malSvrRestSearchService, malSvrRestIndexService, 
		malSvrRestMassDeleteService, 
								alertService, timeoutService) {

/*log.info("inside MalSvrController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside MalSvrController");
	malSvrRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside MalSvrController");
	malSvrRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside MalSvrController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside MalSvrController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside MalSvrController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside MalSvrController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside MalSvrController");
...
scope.startElasticSearch = function(item) {
	...
	malSvrRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	malSvrRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-malSvr.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/malSvr/malSvrSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	malSvrRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			malSvrRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your malSvr have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			malSvrRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) MalSvr entity */
app.factory('MalSvrRestService', function ($resource) {
	return $resource('api/malSvrs/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/malSvrs/:id'},
		'update': { method:'PUT', url: 'api/malSvrs/:id'},
		'delete': { method:'DELETE', url: 'api/malSvrs/:id' }
	});
});

/** REST client for managing Elastic search calls on MalSvr entity */
app.factory('MalSvrRestSearchService', function ($resource) {
	return $resource('api/malSvrs/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing MalSvr entity */
app.factory('MalSvrRestIndexService', function ($resource) {
	return $resource('api/malSvrs/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on MalSvr entity */
app.factory('MalSvrRestMassDeleteService', function ($resource) {
	return $resource('api/malSvrs/mass/:id', {}, {
...
app.factory('MalSvrRestInvRelationService', function ($resource) {
