...
app.controller("TopCalController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "TopCalRestService", "TopCalRestSearchService", 
		"TopCalRestIndexService", "TopCalRestMassDeleteService",
												"$alert", "$timeout", function(scope, window, c, d, log, 
		topCalRestService, topCalRestSearchService, topCalRestIndexService, 
		topCalRestMassDeleteService, 
												alertService, timeoutService) {

/*log.info("inside TopCalController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside TopCalController");
	topCalRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside TopCalController");
	topCalRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside TopCalController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside TopCalController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside TopCalController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside TopCalController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside TopCalController");
...
scope.startElasticSearch = function(item) {
	...
	topCalRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	topCalRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-topCal.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/topCal/topCalSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	topCalRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your topCal have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your topCal haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		topCalRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		topCalRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			topCalRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your topCal have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			topCalRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) TopCal entity */
app.factory('TopCalRestService', function ($resource) {
	return $resource('api/topCals/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/topCals/:id'},
		'update': { method:'PUT', url: 'api/topCals/:id'},
		'delete': { method:'DELETE', url: 'api/topCals/:id' }
	});
});

/** REST client for managing Elastic search calls on TopCal entity */
app.factory('TopCalRestSearchService', function ($resource) {
	return $resource('api/topCals/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing TopCal entity */
app.factory('TopCalRestIndexService', function ($resource) {
	return $resource('api/topCals/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on TopCal entity */
app.factory('TopCalRestMassDeleteService', function ($resource) {
	return $resource('api/topCals/mass/:id', {}, {
...
app.factory('TopCalRestInvRelationService', function ($resource) {
