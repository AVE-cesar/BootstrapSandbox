...
app.controller("FieParController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "FieParRestService", "FieParRestSearchService", 
		"FieParRestIndexService", "FieParRestMassDeleteService",
						"FieRestService",
							"$alert", "$timeout", function(scope, window, c, d, log, 
		fieParRestService, fieParRestSearchService, fieParRestIndexService, 
		fieParRestMassDeleteService, 
						fieRestService,
							alertService, timeoutService) {

/*log.info("inside FieParController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside FieParController");
	fieParRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside FieParController");
	fieParRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside FieParController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside FieParController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside FieParController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside FieParController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside FieParController");
...
scope.startElasticSearch = function(item) {
	...
	fieParRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	fieParRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-fiePar.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/fiePar/fieParSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	fieParRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your fiePar have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your fiePar haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		fieParRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		fieParRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			fieParRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your fiePar have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			fieParRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) FiePar entity */
app.factory('FieParRestService', function ($resource) {
	return $resource('api/fiePars/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/fiePars/:id'},
		'update': { method:'PUT', url: 'api/fiePars/:id'},
		'delete': { method:'DELETE', url: 'api/fiePars/:id' }
	});
});

/** REST client for managing Elastic search calls on FiePar entity */
app.factory('FieParRestSearchService', function ($resource) {
	return $resource('api/fiePars/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing FiePar entity */
app.factory('FieParRestIndexService', function ($resource) {
	return $resource('api/fiePars/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on FiePar entity */
app.factory('FieParRestMassDeleteService', function ($resource) {
	return $resource('api/fiePars/mass/:id', {}, {
...
app.factory('FieParRestInvRelationService', function ($resource) {
