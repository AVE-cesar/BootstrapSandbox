...
app.controller("PlanTableController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "PlanTableRestService", "PlanTableRestSearchService", 
		"PlanTableRestIndexService", "PlanTableRestMassDeleteService",
																															"$alert", "$timeout", function(scope, window, c, d, log, 
		planTableRestService, planTableRestSearchService, planTableRestIndexService, 
		planTableRestMassDeleteService, 
																															alertService, timeoutService) {

/*log.info("inside PlanTableController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside PlanTableController");
	planTableRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside PlanTableController");
	planTableRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside PlanTableController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside PlanTableController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside PlanTableController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside PlanTableController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside PlanTableController");
...
scope.startElasticSearch = function(item) {
	...
	planTableRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	planTableRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-planTable.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/planTable/planTableSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	planTableRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your planTable have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your planTable haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		planTableRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		planTableRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			planTableRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your planTable have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			planTableRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	planTableRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) PlanTable entity */
app.factory('PlanTableRestService', function ($resource) {
	return $resource('api/planTables/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/planTables/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/planTables/:id'},
		'update': { method:'PUT', url: 'api/planTables/:id'},
		'delete': { method:'DELETE', url: 'api/planTables/:id' }
	});
});

/** REST client for managing Elastic search calls on PlanTable entity */
app.factory('PlanTableRestSearchService', function ($resource) {
	return $resource('api/planTables/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing PlanTable entity */
app.factory('PlanTableRestIndexService', function ($resource) {
	return $resource('api/planTables/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on PlanTable entity */
app.factory('PlanTableRestMassDeleteService', function ($resource) {
	return $resource('api/planTables/mass/:id', {}, {
...
app.factory('PlanTableRestInvRelationService', function ($resource) {
