...
app.controller("DiffusionListParController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "DiffusionListParRestService", "DiffusionListParRestSearchService", 
		"DiffusionListParRestIndexService", "DiffusionListParRestMassDeleteService",
							"DiffusionListRestService",
							"$alert", "$timeout", function(scope, window, c, d, log, 
		diffusionListParRestService, diffusionListParRestSearchService, diffusionListParRestIndexService, 
		diffusionListParRestMassDeleteService, 
							diffusionlistRestService,
							alertService, timeoutService) {

/*log.info("inside DiffusionListParController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside DiffusionListParController");
	diffusionListParRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside DiffusionListParController");
	diffusionListParRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside DiffusionListParController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside DiffusionListParController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside DiffusionListParController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside DiffusionListParController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside DiffusionListParController");
...
scope.startElasticSearch = function(item) {
	...
	diffusionListParRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	diffusionListParRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-diffusionListPar.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/diffusionListPar/diffusionListParSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	diffusionListParRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			diffusionListParRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your diffusionListPar have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
...
scope.index = function() {
	diffusionListParRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) DiffusionListPar entity */
app.factory('DiffusionListParRestService', function ($resource) {
	return $resource('api/diffusionListPars/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/diffusionListPars/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/diffusionListPars/:id'},
		'update': { method:'PUT', url: 'api/diffusionListPars/:id'},
		'delete': { method:'DELETE', url: 'api/diffusionListPars/:id' }
	});
});

/** REST client for managing Elastic search calls on DiffusionListPar entity */
app.factory('DiffusionListParRestSearchService', function ($resource) {
	return $resource('api/diffusionListPars/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing DiffusionListPar entity */
app.factory('DiffusionListParRestIndexService', function ($resource) {
	return $resource('api/diffusionListPars/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on DiffusionListPar entity */
app.factory('DiffusionListParRestMassDeleteService', function ($resource) {
	return $resource('api/diffusionListPars/mass/:id', {}, {
...
app.factory('DiffusionListParRestInvRelationService', function ($resource) {
