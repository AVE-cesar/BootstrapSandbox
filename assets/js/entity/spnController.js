...
app.controller("SpnController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "SpnRestService", "SpnRestSearchService", 
		"SpnRestIndexService", "SpnRestMassDeleteService",
											"$alert", "$timeout", function(scope, window, c, d, log, 
		spnRestService, spnRestSearchService, spnRestIndexService, 
		spnRestMassDeleteService, 
											alertService, timeoutService) {

/*log.info("inside SpnController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside SpnController");
	spnRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside SpnController");
	spnRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside SpnController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside SpnController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside SpnController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside SpnController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside SpnController");
...
scope.startElasticSearch = function(item) {
	...
	spnRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	spnRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-spn.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/spn/spnSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	spnRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your spn have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your spn haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		spnRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		spnRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			spnRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your spn have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			spnRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Spn entity */
app.factory('SpnRestService', function ($resource) {
	return $resource('api/spns/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/spns/:id'},
		'update': { method:'PUT', url: 'api/spns/:id'},
		'delete': { method:'DELETE', url: 'api/spns/:id' }
	});
});

/** REST client for managing Elastic search calls on Spn entity */
app.factory('SpnRestSearchService', function ($resource) {
	return $resource('api/spns/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Spn entity */
app.factory('SpnRestIndexService', function ($resource) {
	return $resource('api/spns/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Spn entity */
app.factory('SpnRestMassDeleteService', function ($resource) {
	return $resource('api/spns/mass/:id', {}, {
...
app.factory('SpnRestInvRelationService', function ($resource) {
