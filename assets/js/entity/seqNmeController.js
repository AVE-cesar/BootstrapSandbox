...
app.controller("SeqNmeController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "SeqNmeRestService", "SeqNmeRestSearchService", 
		"SeqNmeRestIndexService", "SeqNmeRestMassDeleteService",
						"$alert", "$timeout", function(scope, window, c, d, log, 
		seqNmeRestService, seqNmeRestSearchService, seqNmeRestIndexService, 
		seqNmeRestMassDeleteService, 
						alertService, timeoutService) {

/*log.info("inside SeqNmeController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside SeqNmeController");
	seqNmeRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside SeqNmeController");
	seqNmeRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside SeqNmeController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside SeqNmeController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside SeqNmeController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside SeqNmeController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside SeqNmeController");
...
scope.startElasticSearch = function(item) {
	...
	seqNmeRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	seqNmeRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-seqNme.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/seqNme/seqNmeSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	seqNmeRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your seqNme have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your seqNme haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		seqNmeRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		seqNmeRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			seqNmeRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your seqNme have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			seqNmeRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) SeqNme entity */
app.factory('SeqNmeRestService', function ($resource) {
	return $resource('api/seqNmes/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/seqNmes/:id'},
		'update': { method:'PUT', url: 'api/seqNmes/:id'},
		'delete': { method:'DELETE', url: 'api/seqNmes/:id' }
	});
});

/** REST client for managing Elastic search calls on SeqNme entity */
app.factory('SeqNmeRestSearchService', function ($resource) {
	return $resource('api/seqNmes/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing SeqNme entity */
app.factory('SeqNmeRestIndexService', function ($resource) {
	return $resource('api/seqNmes/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on SeqNme entity */
app.factory('SeqNmeRestMassDeleteService', function ($resource) {
	return $resource('api/seqNmes/mass/:id', {}, {
...
app.factory('SeqNmeRestInvRelationService', function ($resource) {
