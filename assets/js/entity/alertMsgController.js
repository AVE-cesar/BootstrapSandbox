...
app.controller("AlertMsgController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "AlertMsgRestService", "AlertMsgRestSearchService", 
		"AlertMsgRestIndexService", "AlertMsgRestMassDeleteService",
							"AlertRestService",
									"$alert", "$timeout", function(scope, window, c, d, log, 
		alertMsgRestService, alertMsgRestSearchService, alertMsgRestIndexService, 
		alertMsgRestMassDeleteService, 
							alertRestService,
									alertService, timeoutService) {

/*log.info("inside AlertMsgController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside AlertMsgController");
	alertMsgRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside AlertMsgController");
	alertMsgRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside AlertMsgController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside AlertMsgController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside AlertMsgController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside AlertMsgController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside AlertMsgController");
...
scope.startElasticSearch = function(item) {
	...
	alertMsgRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	alertMsgRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-alertMsg.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/alertMsg/alertMsgSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	alertMsgRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your alertMsg have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your alertMsg haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		alertMsgRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		alertMsgRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			alertMsgRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your alertMsg have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			alertMsgRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	alertMsgRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) AlertMsg entity */
app.factory('AlertMsgRestService', function ($resource) {
	return $resource('api/alertMsgs/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/alertMsgs/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/alertMsgs/:id'},
		'update': { method:'PUT', url: 'api/alertMsgs/:id'},
		'delete': { method:'DELETE', url: 'api/alertMsgs/:id' }
	});
});

/** REST client for managing Elastic search calls on AlertMsg entity */
app.factory('AlertMsgRestSearchService', function ($resource) {
	return $resource('api/alertMsgs/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing AlertMsg entity */
app.factory('AlertMsgRestIndexService', function ($resource) {
	return $resource('api/alertMsgs/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on AlertMsg entity */
app.factory('AlertMsgRestMassDeleteService', function ($resource) {
	return $resource('api/alertMsgs/mass/:id', {}, {
...
app.factory('AlertMsgRestInvRelationService', function ($resource) {
