...
app.controller("VbsFlowUnitMediaAvant2015Controller", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "VbsFlowUnitMediaAvant2015RestService", "VbsFlowUnitMediaAvant2015RestSearchService", 
		"VbsFlowUnitMediaAvant2015RestIndexService", "VbsFlowUnitMediaAvant2015RestMassDeleteService",
					"$alert", "$timeout", function(scope, window, c, d, log, 
		vbsFlowUnitMediaAvant2015RestService, vbsFlowUnitMediaAvant2015RestSearchService, vbsFlowUnitMediaAvant2015RestIndexService, 
		vbsFlowUnitMediaAvant2015RestMassDeleteService, 
					alertService, timeoutService) {

/*log.info("inside VbsFlowUnitMediaAvant2015Controller, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside VbsFlowUnitMediaAvant2015Controller");
	vbsFlowUnitMediaAvant2015RestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside VbsFlowUnitMediaAvant2015Controller");
	vbsFlowUnitMediaAvant2015RestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside VbsFlowUnitMediaAvant2015Controller for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside VbsFlowUnitMediaAvant2015Controller for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside VbsFlowUnitMediaAvant2015Controller for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside VbsFlowUnitMediaAvant2015Controller for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside VbsFlowUnitMediaAvant2015Controller");
...
scope.startElasticSearch = function(item) {
	...
	vbsFlowUnitMediaAvant2015RestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	vbsFlowUnitMediaAvant2015RestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-vbsFlowUnitMediaAvant2015.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/vbsFlowUnitMediaAvant2015/vbsFlowUnitMediaAvant2015Search.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	vbsFlowUnitMediaAvant2015RestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your vbsFlowUnitMediaAvant2015 have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your vbsFlowUnitMediaAvant2015 haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		vbsFlowUnitMediaAvant2015RestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		vbsFlowUnitMediaAvant2015RestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			vbsFlowUnitMediaAvant2015RestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your vbsFlowUnitMediaAvant2015 have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			vbsFlowUnitMediaAvant2015RestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	vbsFlowUnitMediaAvant2015RestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) VbsFlowUnitMediaAvant2015 entity */
app.factory('VbsFlowUnitMediaAvant2015RestService', function ($resource) {
	return $resource('api/vbsFlowUnitMediaAvant2015s/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/vbsFlowUnitMediaAvant2015s/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/vbsFlowUnitMediaAvant2015s/:id'},
		'update': { method:'PUT', url: 'api/vbsFlowUnitMediaAvant2015s/:id'},
		'delete': { method:'DELETE', url: 'api/vbsFlowUnitMediaAvant2015s/:id' }
	});
});

/** REST client for managing Elastic search calls on VbsFlowUnitMediaAvant2015 entity */
app.factory('VbsFlowUnitMediaAvant2015RestSearchService', function ($resource) {
	return $resource('api/vbsFlowUnitMediaAvant2015s/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing VbsFlowUnitMediaAvant2015 entity */
app.factory('VbsFlowUnitMediaAvant2015RestIndexService', function ($resource) {
	return $resource('api/vbsFlowUnitMediaAvant2015s/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on VbsFlowUnitMediaAvant2015 entity */
app.factory('VbsFlowUnitMediaAvant2015RestMassDeleteService', function ($resource) {
	return $resource('api/vbsFlowUnitMediaAvant2015s/mass/:id', {}, {
...
app.factory('VbsFlowUnitMediaAvant2015RestInvRelationService', function ($resource) {
