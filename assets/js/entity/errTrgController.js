...
app.controller("ErrTrgController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "ErrTrgRestService", "ErrTrgRestSearchService", 
		"ErrTrgRestIndexService", "ErrTrgRestMassDeleteService",
							"ErrRestService",
													"$alert", "$timeout", function(scope, window, c, d, log, 
		errTrgRestService, errTrgRestSearchService, errTrgRestIndexService, 
		errTrgRestMassDeleteService, 
							errRestService,
													alertService, timeoutService) {

/*log.info("inside ErrTrgController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside ErrTrgController");
	errTrgRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside ErrTrgController");
	errTrgRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside ErrTrgController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside ErrTrgController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside ErrTrgController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside ErrTrgController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside ErrTrgController");
...
scope.startElasticSearch = function(item) {
	...
	errTrgRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	errTrgRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-errTrg.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/errTrg/errTrgSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	errTrgRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			errTrgRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your errTrg have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			errTrgRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) ErrTrg entity */
app.factory('ErrTrgRestService', function ($resource) {
	return $resource('api/errTrgs/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/errTrgs/:id'},
		'update': { method:'PUT', url: 'api/errTrgs/:id'},
		'delete': { method:'DELETE', url: 'api/errTrgs/:id' }
	});
});

/** REST client for managing Elastic search calls on ErrTrg entity */
app.factory('ErrTrgRestSearchService', function ($resource) {
	return $resource('api/errTrgs/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing ErrTrg entity */
app.factory('ErrTrgRestIndexService', function ($resource) {
	return $resource('api/errTrgs/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on ErrTrg entity */
app.factory('ErrTrgRestMassDeleteService', function ($resource) {
	return $resource('api/errTrgs/mass/:id', {}, {
...
app.factory('ErrTrgRestInvRelationService', function ($resource) {
