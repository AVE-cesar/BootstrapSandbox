...
app.controller("EdmJobReqLogController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "EdmJobReqLogRestService", "EdmJobReqLogRestSearchService", 
		"EdmJobReqLogRestIndexService", "EdmJobReqLogRestMassDeleteService",
																			"$alert", "$timeout", function(scope, window, c, d, log, 
		edmJobReqLogRestService, edmJobReqLogRestSearchService, edmJobReqLogRestIndexService, 
		edmJobReqLogRestMassDeleteService, 
																			alertService, timeoutService) {

/*log.info("inside EdmJobReqLogController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside EdmJobReqLogController");
	edmJobReqLogRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside EdmJobReqLogController");
	edmJobReqLogRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside EdmJobReqLogController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside EdmJobReqLogController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside EdmJobReqLogController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside EdmJobReqLogController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside EdmJobReqLogController");
...
scope.startElasticSearch = function(item) {
	...
	edmJobReqLogRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	edmJobReqLogRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-edmJobReqLog.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/edmJobReqLog/edmJobReqLogSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	edmJobReqLogRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your edmJobReqLog have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your edmJobReqLog haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		edmJobReqLogRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		edmJobReqLogRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			edmJobReqLogRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your edmJobReqLog have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			edmJobReqLogRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	edmJobReqLogRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) EdmJobReqLog entity */
app.factory('EdmJobReqLogRestService', function ($resource) {
	return $resource('api/edmJobReqLogs/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/edmJobReqLogs/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/edmJobReqLogs/:id'},
		'update': { method:'PUT', url: 'api/edmJobReqLogs/:id'},
		'delete': { method:'DELETE', url: 'api/edmJobReqLogs/:id' }
	});
});

/** REST client for managing Elastic search calls on EdmJobReqLog entity */
app.factory('EdmJobReqLogRestSearchService', function ($resource) {
	return $resource('api/edmJobReqLogs/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing EdmJobReqLog entity */
app.factory('EdmJobReqLogRestIndexService', function ($resource) {
	return $resource('api/edmJobReqLogs/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on EdmJobReqLog entity */
app.factory('EdmJobReqLogRestMassDeleteService', function ($resource) {
	return $resource('api/edmJobReqLogs/mass/:id', {}, {
...
app.factory('EdmJobReqLogRestInvRelationService', function ($resource) {
