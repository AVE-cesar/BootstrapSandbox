...
app.controller("LogMalDfuLstController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "LogMalDfuLstRestService", "LogMalDfuLstRestSearchService", 
		"LogMalDfuLstRestIndexService", "LogMalDfuLstRestMassDeleteService",
					"$alert", "$timeout", function(scope, window, c, d, log, 
		logMalDfuLstRestService, logMalDfuLstRestSearchService, logMalDfuLstRestIndexService, 
		logMalDfuLstRestMassDeleteService, 
					alertService, timeoutService) {

/*log.info("inside LogMalDfuLstController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside LogMalDfuLstController");
	logMalDfuLstRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside LogMalDfuLstController");
	logMalDfuLstRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside LogMalDfuLstController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside LogMalDfuLstController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside LogMalDfuLstController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside LogMalDfuLstController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside LogMalDfuLstController");
...
scope.startElasticSearch = function(item) {
	...
	logMalDfuLstRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	logMalDfuLstRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-logMalDfuLst.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/logMalDfuLst/logMalDfuLstSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	logMalDfuLstRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your logMalDfuLst have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your logMalDfuLst haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		logMalDfuLstRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		logMalDfuLstRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			logMalDfuLstRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your logMalDfuLst have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			logMalDfuLstRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	logMalDfuLstRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) LogMalDfuLst entity */
app.factory('LogMalDfuLstRestService', function ($resource) {
	return $resource('api/logMalDfuLsts/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/logMalDfuLsts/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/logMalDfuLsts/:id'},
		'update': { method:'PUT', url: 'api/logMalDfuLsts/:id'},
		'delete': { method:'DELETE', url: 'api/logMalDfuLsts/:id' }
	});
});

/** REST client for managing Elastic search calls on LogMalDfuLst entity */
app.factory('LogMalDfuLstRestSearchService', function ($resource) {
	return $resource('api/logMalDfuLsts/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing LogMalDfuLst entity */
app.factory('LogMalDfuLstRestIndexService', function ($resource) {
	return $resource('api/logMalDfuLsts/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on LogMalDfuLst entity */
app.factory('LogMalDfuLstRestMassDeleteService', function ($resource) {
	return $resource('api/logMalDfuLsts/mass/:id', {}, {
...
app.factory('LogMalDfuLstRestInvRelationService', function ($resource) {
