...
app.controller("SwiftArchiverController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "SwiftArchiverRestService", "SwiftArchiverRestSearchService", 
		"SwiftArchiverRestIndexService", "SwiftArchiverRestMassDeleteService",
						"$alert", "$timeout", function(scope, window, c, d, log, 
		swiftArchiverRestService, swiftArchiverRestSearchService, swiftArchiverRestIndexService, 
		swiftArchiverRestMassDeleteService, 
						alertService, timeoutService) {

/*log.info("inside SwiftArchiverController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside SwiftArchiverController");
	swiftArchiverRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside SwiftArchiverController");
	swiftArchiverRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside SwiftArchiverController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside SwiftArchiverController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside SwiftArchiverController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside SwiftArchiverController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside SwiftArchiverController");
...
scope.startElasticSearch = function(item) {
	...
	swiftArchiverRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	swiftArchiverRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-swiftArchiver.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/swiftArchiver/swiftArchiverSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	swiftArchiverRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your swiftArchiver have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your swiftArchiver haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		swiftArchiverRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		swiftArchiverRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			swiftArchiverRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your swiftArchiver have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			swiftArchiverRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	swiftArchiverRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) SwiftArchiver entity */
app.factory('SwiftArchiverRestService', function ($resource) {
	return $resource('api/swiftArchivers/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/swiftArchivers/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/swiftArchivers/:id'},
		'update': { method:'PUT', url: 'api/swiftArchivers/:id'},
		'delete': { method:'DELETE', url: 'api/swiftArchivers/:id' }
	});
});

/** REST client for managing Elastic search calls on SwiftArchiver entity */
app.factory('SwiftArchiverRestSearchService', function ($resource) {
	return $resource('api/swiftArchivers/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing SwiftArchiver entity */
app.factory('SwiftArchiverRestIndexService', function ($resource) {
	return $resource('api/swiftArchivers/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on SwiftArchiver entity */
app.factory('SwiftArchiverRestMassDeleteService', function ($resource) {
	return $resource('api/swiftArchivers/mass/:id', {}, {
...
app.factory('SwiftArchiverRestInvRelationService', function ($resource) {
