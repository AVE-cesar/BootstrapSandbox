...
app.controller("TrfParController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "TrfParRestService", "TrfParRestSearchService", 
		"TrfParRestIndexService", "TrfParRestMassDeleteService",
						"RptSpnLnkRestService",
							"$alert", "$timeout", function(scope, window, c, d, log, 
		trfParRestService, trfParRestSearchService, trfParRestIndexService, 
		trfParRestMassDeleteService, 
						rptspnlnkRestService,
							alertService, timeoutService) {

/*log.info("inside TrfParController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside TrfParController");
	trfParRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside TrfParController");
	trfParRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside TrfParController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside TrfParController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside TrfParController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside TrfParController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside TrfParController");
...
scope.startElasticSearch = function(item) {
	...
	trfParRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	trfParRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-trfPar.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/trfPar/trfParSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	trfParRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your trfPar have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your trfPar haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		trfParRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		trfParRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			trfParRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your trfPar have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			trfParRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) TrfPar entity */
app.factory('TrfParRestService', function ($resource) {
	return $resource('api/trfPars/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/trfPars/:id'},
		'update': { method:'PUT', url: 'api/trfPars/:id'},
		'delete': { method:'DELETE', url: 'api/trfPars/:id' }
	});
});

/** REST client for managing Elastic search calls on TrfPar entity */
app.factory('TrfParRestSearchService', function ($resource) {
	return $resource('api/trfPars/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing TrfPar entity */
app.factory('TrfParRestIndexService', function ($resource) {
	return $resource('api/trfPars/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on TrfPar entity */
app.factory('TrfParRestMassDeleteService', function ($resource) {
	return $resource('api/trfPars/mass/:id', {}, {
...
app.factory('TrfParRestInvRelationService', function ($resource) {
