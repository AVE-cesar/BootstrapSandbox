...
app.controller("RecipientsController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "RecipientsRestService", "RecipientsRestSearchService", 
		"RecipientsRestIndexService", "RecipientsRestMassDeleteService",
							"UploadsRestService",
							"$alert", "$timeout", function(scope, window, c, d, log, 
		recipientsRestService, recipientsRestSearchService, recipientsRestIndexService, 
		recipientsRestMassDeleteService, 
							uploadsRestService,
							alertService, timeoutService) {

/*log.info("inside RecipientsController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside RecipientsController");
	recipientsRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside RecipientsController");
	recipientsRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside RecipientsController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside RecipientsController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside RecipientsController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside RecipientsController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside RecipientsController");
...
scope.startElasticSearch = function(item) {
	...
	recipientsRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	recipientsRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-recipients.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/recipients/recipientsSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	recipientsRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your recipients have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your recipients haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		recipientsRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		recipientsRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			recipientsRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your recipients have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			recipientsRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	recipientsRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Recipients entity */
app.factory('RecipientsRestService', function ($resource) {
	return $resource('api/recipientss/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/recipientss/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/recipientss/:id'},
		'update': { method:'PUT', url: 'api/recipientss/:id'},
		'delete': { method:'DELETE', url: 'api/recipientss/:id' }
	});
});

/** REST client for managing Elastic search calls on Recipients entity */
app.factory('RecipientsRestSearchService', function ($resource) {
	return $resource('api/recipientss/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Recipients entity */
app.factory('RecipientsRestIndexService', function ($resource) {
	return $resource('api/recipientss/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Recipients entity */
app.factory('RecipientsRestMassDeleteService', function ($resource) {
	return $resource('api/recipientss/mass/:id', {}, {
...
app.factory('RecipientsRestInvRelationService', function ($resource) {
