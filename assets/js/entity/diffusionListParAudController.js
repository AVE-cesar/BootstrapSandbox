...
app.controller("DiffusionListParAudController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "DiffusionListParAudRestService", "DiffusionListParAudRestSearchService", 
		"DiffusionListParAudRestIndexService", "DiffusionListParAudRestMassDeleteService",
										"$alert", "$timeout", function(scope, window, c, d, log, 
		diffusionListParAudRestService, diffusionListParAudRestSearchService, diffusionListParAudRestIndexService, 
		diffusionListParAudRestMassDeleteService, 
										alertService, timeoutService) {

/*log.info("inside DiffusionListParAudController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside DiffusionListParAudController");
	diffusionListParAudRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside DiffusionListParAudController");
	diffusionListParAudRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside DiffusionListParAudController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside DiffusionListParAudController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside DiffusionListParAudController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside DiffusionListParAudController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside DiffusionListParAudController");
...
scope.startElasticSearch = function(item) {
	...
	diffusionListParAudRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	diffusionListParAudRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-diffusionListParAud.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/diffusionListParAud/diffusionListParAudSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	diffusionListParAudRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your diffusionListParAud have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your diffusionListParAud haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		diffusionListParAudRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		diffusionListParAudRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			diffusionListParAudRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your diffusionListParAud have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			diffusionListParAudRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	diffusionListParAudRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) DiffusionListParAud entity */
app.factory('DiffusionListParAudRestService', function ($resource) {
	return $resource('api/diffusionListParAuds/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/diffusionListParAuds/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/diffusionListParAuds/:id'},
		'update': { method:'PUT', url: 'api/diffusionListParAuds/:id'},
		'delete': { method:'DELETE', url: 'api/diffusionListParAuds/:id' }
	});
});

/** REST client for managing Elastic search calls on DiffusionListParAud entity */
app.factory('DiffusionListParAudRestSearchService', function ($resource) {
	return $resource('api/diffusionListParAuds/search/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing DiffusionListParAud entity */
app.factory('DiffusionListParAudRestIndexService', function ($resource) {
	return $resource('api/diffusionListParAuds/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on DiffusionListParAud entity */
app.factory('DiffusionListParAudRestMassDeleteService', function ($resource) {
	return $resource('api/diffusionListParAuds/mass/:id', {}, {
...
app.factory('DiffusionListParAudRestInvRelationService', function ($resource) {
